...
#ifdef __VIVADO_SYNTH__
#endif //__VIVADO_SYNTH__
  // M_compute_output_3 read pattern: { compute_output[root = 0, c] -> M[c] : 0 <= c <= 9 }
...
#ifdef __VIVADO_SYNTH__
#endif //__VIVADO_SYNTH__
  // M_compute_output_4 read pattern: { compute_output[root = 0, c] -> M[1 + c] : 0 <= c <= 8; compute_output[root = 0, c = 9] -> M[9] }
  // Read schedule : { compute_output[root = 0, c] -> [2 + c, 1] : 0 <= c <= 9 }
  // Write schedule: { get_input[root = 0, p] -> [p, 0] : 0 <= p <= 9 }
...
#ifdef __VIVADO_SYNTH__
#endif //__VIVADO_SYNTH__
  // M_compute_output_5 read pattern: { compute_output[root = 0, c] -> M[9] : 8 <= c <= 9; compute_output[root = 0, c] -> M[2 + c] : 0 <= c <= 7 }
  // Read schedule : { compute_output[root = 0, c] -> [2 + c, 1] : 0 <= c <= 9 }
  // Write schedule: { get_input[root = 0, p] -> [p, 0] : 0 <= p <= 9 }
  // DD fold: {  }
