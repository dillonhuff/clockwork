...
inline void M_write_2_write(hw_uint<32> & M_write_2, M_write_2_cache& M_write_2_delay) {
	M_write_2_delay.push(M_write_2);
}

inline hw_uint<32>  M_read0_5_select(M_write_2_cache& M_write_2_delay
, int root, int c) {
	hw_uint<32>  value_M_write_2 = M_write_2_delay.peek((root == 0 && c >= 0 && 7 - c >= 0) ? (2) : (-8 + c == 0 && root == 0) ? (1) : 0);
	return value_M_write_2;
}

inline hw_uint<32>  M_read0_6_select(M_write_2_cache& M_write_2_delay
, int root, int c) {
	hw_uint<32>  value_M_write_2 = M_write_2_delay.peek((root == 0 && c >= 0 && 7 - c >= 0) ? (1) : 0);
	return value_M_write_2;
}

inline hw_uint<32>  M_read0_7_select(M_write_2_cache& M_write_2_delay
, int root, int c) {
	hw_uint<32>  value_M_write_2 = M_write_2_delay.peek_0();
	return value_M_write_2;
}
...
inline hw_uint<96> M_read0_read_bundle_read(M_write_2_cache& M_write_2_delay, int root, int c) {
	hw_uint<96> result;
	hw_uint<32>  M_read0_5_res = M_read0_5_select(M_write_2_delay, root, c);
	set_at<0, 96>(result, M_read0_5_res);
	hw_uint<32>  M_read0_6_res = M_read0_6_select(M_write_2_delay, root, c);
	set_at<32, 96>(result, M_read0_6_res);
	hw_uint<32>  M_read0_7_res = M_read0_7_select(M_write_2_delay, root, c);
...
// write_write
//	M_write_2
inline void M_write_write_bundle_write(hw_uint<32>& write_write, M_write_2_cache& M_write_2_delay) {
	hw_uint<32>  M_write_2_res = write_write.extract<0, 31>();
	M_write_2_write(M_write_2_res, M_write_2_delay);
}
...
inline void T_read0_write_bundle_write(hw_uint<96>& read0_write, T_read0_4_cache& T_read0_4_delay) {
	...
	T_read0_4_write(T_read0_4_res, T_read0_4_delay);
}
...
// Operation logic
inline void compute_out(T_read0_4_cache& T_read0_4, HWStream<hw_uint<32> >& /* num ports = 1 */out, int root, int c) {
	// Consume: T
...
inline void write(HWStream<hw_uint<32> >& /* num ports = 1 */in, M_write_2_cache& M_write_2, int root, int p) {
	// Consume: in
	auto in_p_value = in.read();
	// Produce: M
	M_write_write_bundle_write(in_p_value, M_write_2);
}

inline void read0(M_write_2_cache& M_write_2, T_read0_4_cache& T_read0_4, int root, int c) {
	// Consume: M
	auto M_min_lp_c_c__9_rp__value = M_read0_read_bundle_read(M_write_2/* source_delay */, root, c);
	// Produce: T
	T_read0_write_bundle_write(M_min_lp_c_c__9_rp__value, T_read0_4);
}

// Driver function
void conv_1d_bc(HWStream<hw_uint<32> >& /* num ports = 1 */in, HWStream<hw_uint<32> >& /* num ports = 1 */out) {
