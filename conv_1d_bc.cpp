...
struct M_write_4_to_M_read0_1_cache {
  ...
  hw_uint<32>  read(int d0) {    return M_write_4_to_M_read0_1_store[(d0)];  }

  void write(hw_uint<32> & value, int d0) {
    M_write_4_to_M_read0_1_store[(d0)] = value;
  }
...
struct M_write_4_merged_banks_2_cache {
	// RAM Box: {[0, 9]}
	// Capacity: 3
	// # of read delays: 2
#ifdef __VIVADO_SYNTH__
...
#endif // __VIVADO_SYNTH__
  hw_uint<32>  read(int d0) {    return M_write_4_merged_banks_2_store[(d0)];  }

  void write(hw_uint<32> & value, int d0) {
    M_write_4_merged_banks_2_store[(d0)] = value;
  }
...
#pragma HLS dependence variable=f inter false
		...
		cout << "Error: Unsupported offset in M_write_4_merged_banks_2: " << offset << endl;
#endif // __VIVADO_SYNTH__
		assert(false);
		return 0;
...
struct M_cache {
  M_write_4_to_M_read0_1_cache M_write_4_to_M_read0_1;
  M_write_4_merged_banks_2_cache M_write_4_merged_banks_2;
};



inline void M_write_4_write(hw_uint<32> & M_write_4, M_cache& M, int root, int p) {
  M.M_write_4_to_M_read0_1.push(M_write_4);
  M.M_write_4_merged_banks_2.push(M_write_4);
}
...
#pragma HLS dependence array inter false
  ...
  // M_read0_1 read pattern: { read0[root = 0, c] -> M[c] : 0 <= c <= 9 }
	auto value_M_write_4 = M.M_write_4_to_M_read0_1.peek(/* Needs general delay string */ (root == 0 && c >= 0 && 7 - c >= 0) ? (2) : (-8 + c == 0 && root == 0) ? (1) : 0);
...
#pragma HLS dependence array inter false
  ...
  // M_read0_2 read pattern: { read0[root = 0, c] -> M[1 + c] : 0 <= c <= 8; read0[root = 0, c = 9] -> M[9] }
	auto value_M_write_4 = M.M_write_4_merged_banks_2.peek(/* Needs general delay string */ (root == 0 && c >= 0 && 7 - c >= 0) ? (1) : 0);
	return value_M_write_4;
}
...
#pragma HLS dependence array inter false
  ...
  // M_read0_3 read pattern: { read0[root = 0, c] -> M[9] : 8 <= c <= 9; read0[root = 0, c] -> M[2 + c] : 0 <= c <= 7 }
	auto value_M_write_4 = M.M_write_4_merged_banks_2.peek_0();
	return value_M_write_4;
}
