...
struct M_write_0_to_M_read0_5_cache {
	...
	inline hw_uint<32>  peek(const int offset) {
		...
		cout << "Error: Unsupported offset in M_write_0_to_M_read0_5: " << offset << endl;
...
inline hw_uint<32>  M_read0_5_select(M_cache& M, int root, int c) {
  ...
  // M_read0_5 read pattern: { read0[root = 0, c] -> M[c] : 0 <= c <= 9 }
	auto value_M_write_0 = M.M_write_0_to_M_read0_5.peek(/* Needs general delay string */ (root == 0 && c >= 0 && 7 - c >= 0) ? (2) : (-8 + c == 0 && root == 0) ? (1) : 0);
