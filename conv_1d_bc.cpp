...
inline void M_write_4_write(hw_uint<32> & M_write_4, M_write_4_cache& M_write_4_delay) {
	M_write_4_delay.push(M_write_4);
}

inline hw_uint<32>  M_read0_1_select(M_write_4_cache& M_write_4_delay
, int root, int c) {
	hw_uint<32>  value_M_write_4 = M_write_4_delay.peek((root == 0 && c >= 0 && 7 - c >= 0) ? (2) : (-8 + c == 0 && root == 0) ? (1) : 0);
	return value_M_write_4;
}

inline hw_uint<32>  M_read0_2_select(M_write_4_cache& M_write_4_delay
, int root, int c) {
	hw_uint<32>  value_M_write_4 = M_write_4_delay.peek((root == 0 && c >= 0 && 7 - c >= 0) ? (1) : 0);
	return value_M_write_4;
}

inline hw_uint<32>  M_read0_3_select(M_write_4_cache& M_write_4_delay
, int root, int c) {
	hw_uint<32>  value_M_write_4 = M_write_4_delay.peek_0();
	return value_M_write_4;
}
...
inline hw_uint<96> M_read0_read_bundle_read(M_write_4_cache& M_write_4_delay, int root, int c) {
	hw_uint<96> result;
	hw_uint<32>  M_read0_1_res = M_read0_1_select(M_write_4_delay, root, c);
	set_at<0, 96>(result, M_read0_1_res);
	hw_uint<32>  M_read0_2_res = M_read0_2_select(M_write_4_delay, root, c);
	set_at<32, 96>(result, M_read0_2_res);
	hw_uint<32>  M_read0_3_res = M_read0_3_select(M_write_4_delay, root, c);
...
// write_write
//	M_write_4
inline void M_write_write_bundle_write(hw_uint<32>& write_write, M_write_4_cache& M_write_4_delay) {
	hw_uint<32>  M_write_4_res = write_write.extract<0, 31>();
	M_write_4_write(M_write_4_res, M_write_4_delay);
}
...
inline void read0(M_write_4_cache& M_write_4, T_read0_0_cache& T_read0_0, int root, int c) {
	// Consume: M
	auto M_min_lp_c_c__9_rp__value = M_read0_read_bundle_read(M_write_4/* source_delay */, root, c);
	// Produce: T
	T_read0_write_bundle_write(M_min_lp_c_c__9_rp__value, T_read0_0);
}

inline void write(HWStream<hw_uint<32> >& /* buffer_args num ports = 1 */in, M_write_4_cache& M_write_4, int root, int p) {
	// Consume: in
	auto in_p_value = in.read();
	// Produce: M
	M_write_write_bundle_write(in_p_value, M_write_4);
}

inline void compute_out(T_read0_0_cache& T_read0_0, HWStream<hw_uint<32> >& /* buffer_args num ports = 1 */out, int root, int c) {
	// Consume: T
...
void conv_1d_bc(HWStream<hw_uint<32> >& /* no bundle get_args num ports = 1 */in, HWStream<hw_uint<32> >& /* no bundle get_args num ports = 1 */out) {
  ...
  T_cache T;
	for (int c0 = 0; c0 <= 11; c0 += 1) {
	  if (c0 <= 9)
	    write(in, M.M_write_4, 0, c0);
	  if (c0 >= 2) {
	    read0(M.M_write_4, T.T_read0_0, 0, c0 - 2);
	    compute_out(T.T_read0_0, out, 0, c0 - 2);
	  }
	}
