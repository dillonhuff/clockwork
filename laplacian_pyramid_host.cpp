...
int main(int argc, char **argv) {
  ...
  const int op_hcompute_hw_input_global_wrapper_stencil_read_pipe0_DATA_SIZE = num_epochs*5776;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "laplacian_pyramid_accel", &err));
      valid_device++;
      break;
    }
  }
