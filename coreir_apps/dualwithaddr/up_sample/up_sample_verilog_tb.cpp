...
#include <fstream>
#include "verilated.h"
#include "Vup_sample.h"

int main() {
  ofstream fout("regression_result_up_sample_verilog.txt");
  ...
  // Loading input data
  srand(1);
  // cmap    : { op_hcompute_hw_input_stencil[root = 0, hw_input_s0_y, hw_input_s0_x] -> input_copy_stencil[hw_input_s0_x, hw_input_s0_y, 0] : 0 <= hw_input_s0_y <= 63 and 0 <= hw_input_s0_x <= 63 }
  // read map: { input_copy_stencil[i0, i1, 0] -> op_hcompute_hw_input_stencil[root = 0, hw_input_s0_y = i1, hw_input_s0_x = i0] : 0 <= i0 <= 63 and 0 <= i1 <= 63 }
  // rng     : { op_hcompute_hw_input_stencil[root = 0, hw_input_s0_y, hw_input_s0_x] : 0 <= hw_input_s0_y <= 63 and 0 <= hw_input_s0_x <= 63 }
  // rng card: { 4096 }
