...
int main(int argc, char **argv) {
  ...
  const int gp_fpga_cata_4_update_0_write_pipe0_DATA_SIZE = num_epochs*32400;
  const int gp_fpga_cata_4_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t gp_fpga_cata_4_update_0_write_pipe0_size_bytes = gp_fpga_cata_4_update_0_write_pipe0_BYTES_PER_PIXEL * gp_fpga_cata_4_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += gp_fpga_cata_4_update_0_write_pipe0_size_bytes;
  const int in_update_0_read_pipe0_DATA_SIZE = num_epochs*2108780;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > gp_fpga_cata_4_update_0_write_pipe0(gp_fpga_cata_4_update_0_write_pipe0_size_bytes);
  ...
  for (int i = 0; i < gp_fpga_cata_4_update_0_write_pipe0_DATA_SIZE; i++) {
    ((uint16_t*) (gp_fpga_cata_4_update_0_write_pipe0.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "gp_fpga_cata_4_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer gp_fpga_cata_4_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, gp_fpga_cata_4_update_0_write_pipe0_size_bytes, gp_fpga_cata_4_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, gp_fpga_cata_4_update_0_write_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({gp_fpga_cata_4_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
{
    std::ofstream regression_result("gp_fpga_cata_4_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < gp_fpga_cata_4_update_0_write_pipe0_DATA_SIZE; i++) {
      regression_result << ((uint16_t*) (gp_fpga_cata_4_update_0_write_pipe0.data()))[i] << std::endl;
    }
}
