...
// Operation logic
inline void get_input(HWStream<hw_uint<32> >& /* num ports = 1 */in, M_get_input_0_cache& M_get_input_0, int root, int p) {
	// Consume: in
	auto in_p_value = in.read();
	// Produce: M
	M_get_input_write_bundle_write(in_p_value, M_get_input_0);
}

inline void compute_output(M_get_input_0_cache& M_get_input_0, HWStream<hw_uint<32> >& /* num ports = 1 */out, int root, int c) {
	// Consume: M
...
// Driver function
void conv_1d(HWStream<hw_uint<32> >& /* num ports = 1 */in, HWStream<hw_uint<32> >& /* num ports = 1 */out) {
