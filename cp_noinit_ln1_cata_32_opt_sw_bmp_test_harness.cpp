...
#include <fstream>
#include <vector>
#include "cp_noinit_ln1_cata_32_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<512> > raw_update_0_read_channel;
  HWStream<hw_uint<512> > cp_noinit_ln1_cata_32_update_0_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 2080 / 32; cl++) {
      hw_uint<512> packed;
      {
      int c = 32*cl + 0;
  ...
  cp_noinit_ln1_cata_32_opt(raw_update_0_read_channel, cp_noinit_ln1_cata_32_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = cp_noinit_ln1_cata_32_update_0_write_channel.read();
  ...
  output.save_image("./images/cp_noinit_ln1_cata_32_opt_bmp_out.bmp");
}
