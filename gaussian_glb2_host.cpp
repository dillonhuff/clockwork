...
int main(int argc, char **argv) {
  ...
  const int op_hcompute_hw_input_global_wrapper_glb_stencil_read_pipe0_DATA_SIZE = num_epochs*25344;
  ...
  const int op_hcompute_hw_input_global_wrapper_glb_stencil_1_read_pipe0_DATA_SIZE = num_epochs*25344;
  ...
  const int op_hcompute_hw_output_global_wrapper_glb_stencil_write_pipe0_DATA_SIZE = num_epochs*24892;
  ...
  const int op_hcompute_hw_output_global_wrapper_glb_stencil_1_write_pipe0_DATA_SIZE = num_epochs*24892;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "gaussian_glb2_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, op_hcompute_hw_input_global_wrapper_glb_stencil_read_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, op_hcompute_hw_output_global_wrapper_glb_stencil_1_write_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(3, op_hcompute_hw_output_global_wrapper_glb_stencil_write_pipe0_ocl_buf));
