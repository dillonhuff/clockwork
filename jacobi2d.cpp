#ifndef __VIVADO_SYNTH__
#include <fstream>
using namespace std;

  // Debug utility
  ofstream* global_debug_handle;

#endif //__VIVADO_SYNTH__
#include "conv_3x3.h"

#include "hw_classes.h"

struct I_I_id0_0_merged_banks_5_cache {
	// RAM Box: {[0, 31], [0, 31]}
	// Capacity: 65
	// # of read delays: 5
	hw_uint<32>  f0;
	fifo<hw_uint<32> , 30> f1;
	hw_uint<32>  f2;
	hw_uint<32>  f4;
	hw_uint<32>  f6;
	fifo<hw_uint<32> , 30> f7;
	hw_uint<32>  f8;


	inline hw_uint<32>  peek_0() {
		return f0;
	}

	inline hw_uint<32>  peek_30() {
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=f1 inter false
#endif //__VIVADO_SYNTH__
		return f1.back();
	}

	inline hw_uint<32>  peek_31() {
		return f2;
	}

	inline hw_uint<32>  peek_32() {
		return f4;
	}

	inline hw_uint<32>  peek_33() {
		return f6;
	}

	inline hw_uint<32>  peek_63() {
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=f7 inter false
#endif //__VIVADO_SYNTH__
		return f7.back();
	}

	inline hw_uint<32>  peek_64() {
		return f8;
	}



	inline void push(const hw_uint<32>  value) {
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=f8 inter false
#endif //__VIVADO_SYNTH__
    // cap: 1 reading from capacity: 30
    f8 = f7.back();
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=f7 inter false
#endif //__VIVADO_SYNTH__
    // cap: 30 reading from capacity: 1
    f7.push(f6);
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=f6 inter false
#endif //__VIVADO_SYNTH__
    // cap: 1 reading from capacity: 1
    f6 = f4;
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=f4 inter false
#endif //__VIVADO_SYNTH__
    // cap: 1 reading from capacity: 1
    f4 = f2;
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=f2 inter false
#endif //__VIVADO_SYNTH__
    // cap: 1 reading from capacity: 30
    f2 = f1.back();
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=f1 inter false
#endif //__VIVADO_SYNTH__
    // cap: 30 reading from capacity: 1
    f1.push(f0);
    // cap: 1
    f0 = value;
	}

};

struct I_cache {
  I_I_id0_0_merged_banks_5_cache I_I_id0_0_merged_banks_5;
};



inline void I_I_id0_0_write(hw_uint<32> & I_I_id0_0, I_cache& I, int root, int id1, int id0) {
  I.I_I_id0_0_merged_banks_5.push(I_I_id0_0);
}

inline hw_uint<32>  I_out_jacobi2d_compute0_3_select(I_cache& I, int root, int d1, int d0) {
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=I inter false
#endif //__VIVADO_SYNTH__
  // I_out_jacobi2d_compute0_3 read pattern: { out_jacobi2d_compute0[root = 0, d1, d0] -> I[d0, 1 + d1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Read schedule : { out_jacobi2d_compute0[root = 0, d1, d0] -> [1 + d1, d0, 1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Write schedule: { I_id0[root = 0, id1, id0] -> [id1, id0, 0] : 0 <= id1 <= 31 and 0 <= id0 <= 31 }
  // DD fold: {  }
  auto value_I_I_id0_0 = I.I_I_id0_0_merged_banks_5.peek_0();
  return value_I_I_id0_0;
#ifndef __VIVADO_SYNTH__
	cout << "Error: Unsupported offsets: " << " root = " << root  << " d1 = " << d1  << " d0 = " << d0  << endl;
	assert(false);
	return 0;
#endif //__VIVADO_SYNTH__
}

inline hw_uint<32>  I_out_jacobi2d_compute0_4_select(I_cache& I, int root, int d1, int d0) {
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=I inter false
#endif //__VIVADO_SYNTH__
  // I_out_jacobi2d_compute0_4 read pattern: { out_jacobi2d_compute0[root = 0, d1, d0] -> I[1 + d0, d1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Read schedule : { out_jacobi2d_compute0[root = 0, d1, d0] -> [1 + d1, d0, 1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Write schedule: { I_id0[root = 0, id1, id0] -> [id1, id0, 0] : 0 <= id1 <= 31 and 0 <= id0 <= 31 }
  // DD fold: { out_jacobi2d_compute0[root, d1, d0] -> 31 : root = 0 and 0 < d1 <= 30 and 0 < d0 <= 29; out_jacobi2d_compute0[root, d1, d0] -> (1 + d0) : root = 0 and d0 = 30 and 0 < d1 <= 30 }
  auto value_I_I_id0_0 = I.I_I_id0_0_merged_banks_5.peek_31();
  return value_I_I_id0_0;
#ifndef __VIVADO_SYNTH__
	cout << "Error: Unsupported offsets: " << " root = " << root  << " d1 = " << d1  << " d0 = " << d0  << endl;
	assert(false);
	return 0;
#endif //__VIVADO_SYNTH__
}

inline hw_uint<32>  I_out_jacobi2d_compute0_5_select(I_cache& I, int root, int d1, int d0) {
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=I inter false
#endif //__VIVADO_SYNTH__
  // I_out_jacobi2d_compute0_5 read pattern: { out_jacobi2d_compute0[root = 0, d1, d0] -> I[d0, d1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Read schedule : { out_jacobi2d_compute0[root = 0, d1, d0] -> [1 + d1, d0, 1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Write schedule: { I_id0[root = 0, id1, id0] -> [id1, id0, 0] : 0 <= id1 <= 31 and 0 <= id0 <= 31 }
  // DD fold: { out_jacobi2d_compute0[root, d1, d0] -> 32 : root = 0 and 0 < d1 <= 30 and 0 < d0 <= 30 }
  auto value_I_I_id0_0 = I.I_I_id0_0_merged_banks_5.peek_32();
  return value_I_I_id0_0;
#ifndef __VIVADO_SYNTH__
	cout << "Error: Unsupported offsets: " << " root = " << root  << " d1 = " << d1  << " d0 = " << d0  << endl;
	assert(false);
	return 0;
#endif //__VIVADO_SYNTH__
}

inline hw_uint<32>  I_out_jacobi2d_compute0_6_select(I_cache& I, int root, int d1, int d0) {
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=I inter false
#endif //__VIVADO_SYNTH__
  // I_out_jacobi2d_compute0_6 read pattern: { out_jacobi2d_compute0[root = 0, d1, d0] -> I[d0, -1 + d1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Read schedule : { out_jacobi2d_compute0[root = 0, d1, d0] -> [1 + d1, d0, 1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Write schedule: { I_id0[root = 0, id1, id0] -> [id1, id0, 0] : 0 <= id1 <= 31 and 0 <= id0 <= 31 }
  // DD fold: { out_jacobi2d_compute0[root, d1, d0] -> 64 : root = 0 and 0 < d1 <= 30 and 0 < d0 <= 30 }
  auto value_I_I_id0_0 = I.I_I_id0_0_merged_banks_5.peek_64();
  return value_I_I_id0_0;
#ifndef __VIVADO_SYNTH__
	cout << "Error: Unsupported offsets: " << " root = " << root  << " d1 = " << d1  << " d0 = " << d0  << endl;
	assert(false);
	return 0;
#endif //__VIVADO_SYNTH__
}

inline hw_uint<32>  I_out_jacobi2d_compute0_7_select(I_cache& I, int root, int d1, int d0) {
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=I inter false
#endif //__VIVADO_SYNTH__
  // I_out_jacobi2d_compute0_7 read pattern: { out_jacobi2d_compute0[root = 0, d1, d0] -> I[-1 + d0, d1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Read schedule : { out_jacobi2d_compute0[root = 0, d1, d0] -> [1 + d1, d0, 1] : 0 < d1 <= 30 and 0 < d0 <= 30 }
  // Write schedule: { I_id0[root = 0, id1, id0] -> [id1, id0, 0] : 0 <= id1 <= 31 and 0 <= id0 <= 31 }
  // DD fold: { out_jacobi2d_compute0[root, d1, d0] -> 33 : root = 0 and 0 < d1 <= 30 and 0 < d0 <= 30 }
  auto value_I_I_id0_0 = I.I_I_id0_0_merged_banks_5.peek_33();
  return value_I_I_id0_0;
#ifndef __VIVADO_SYNTH__
	cout << "Error: Unsupported offsets: " << " root = " << root  << " d1 = " << d1  << " d0 = " << d0  << endl;
	assert(false);
	return 0;
#endif //__VIVADO_SYNTH__
}

// # of bundles = 2
// I_id0_write
//	I_I_id0_0
inline void I_I_id0_write_bundle_write(hw_uint<32>& I_id0_write, I_cache& I, int root, int id1, int id0) {
	hw_uint<32>  I_I_id0_0_res = I_id0_write.extract<0, 31>();
	I_I_id0_0_write(I_I_id0_0_res, I, root, id1, id0);
}

// out_jacobi2d_compute0_read
//	I_out_jacobi2d_compute0_3
//	I_out_jacobi2d_compute0_4
//	I_out_jacobi2d_compute0_5
//	I_out_jacobi2d_compute0_6
//	I_out_jacobi2d_compute0_7
inline hw_uint<160> I_out_jacobi2d_compute0_read_bundle_read(I_cache& I, int root, int d1, int d0) {
	hw_uint<160> result;
	hw_uint<32>  I_out_jacobi2d_compute0_3_res = I_out_jacobi2d_compute0_3_select(I, root, d1, d0);
	set_at<0, 160>(result, I_out_jacobi2d_compute0_3_res);
	hw_uint<32>  I_out_jacobi2d_compute0_4_res = I_out_jacobi2d_compute0_4_select(I, root, d1, d0);
	set_at<32, 160>(result, I_out_jacobi2d_compute0_4_res);
	hw_uint<32>  I_out_jacobi2d_compute0_5_res = I_out_jacobi2d_compute0_5_select(I, root, d1, d0);
	set_at<64, 160>(result, I_out_jacobi2d_compute0_5_res);
	hw_uint<32>  I_out_jacobi2d_compute0_6_res = I_out_jacobi2d_compute0_6_select(I, root, d1, d0);
	set_at<96, 160>(result, I_out_jacobi2d_compute0_6_res);
	hw_uint<32>  I_out_jacobi2d_compute0_7_res = I_out_jacobi2d_compute0_7_select(I, root, d1, d0);
	set_at<128, 160>(result, I_out_jacobi2d_compute0_7_res);
	return result;
}



// Operation logic
inline void I_id0(HWStream<hw_uint<32> >& /* buffer_args num ports = 1 */in, I_cache& I, int root, int id1, int id0) {
	// Consume: in
	auto in_id0_c__id1_value = in.read();
	auto compute_result = id(in_id0_c__id1_value);
	// Produce: I
	I_I_id0_write_bundle_write(compute_result, I, root, id1, id0);
#ifndef __VIVADO_SYNTH__
  hw_uint<32> debug_compute_result(compute_result);
  hw_uint<32> debug_compute_result_lane_0;
  set_at<0, 32>(debug_compute_result_lane_0, debug_compute_result.extract<0, 31>());
  *global_debug_handle << "I_id0," << (1*root + 0) << ", " << id1<< "," << id0<< "," <<  debug_compute_result_lane_0 << endl;
#endif //__VIVADO_SYNTH__
}

inline void out_jacobi2d_compute0(I_cache& I, HWStream<hw_uint<32> >& /* buffer_args num ports = 1 */out, int root, int d1, int d0) {
	// Consume: I
	auto I_d0__p__0_c__d1__p__1_value = I_out_jacobi2d_compute0_read_bundle_read(I/* source_delay */, root, d1, d0);
#ifndef __VIVADO_SYNTH__
  *global_debug_handle << "out_jacobi2d_compute0_I," << root<< "," << d1<< "," << d0<< "," <<  I_d0__p__0_c__d1__p__1_value << endl;
#endif //__VIVADO_SYNTH__
	auto compute_result = jacobi2d_compute(I_d0__p__0_c__d1__p__1_value);
	// Produce: out
	out.write(compute_result);
#ifndef __VIVADO_SYNTH__
  hw_uint<32> debug_compute_result(compute_result);
  hw_uint<32> debug_compute_result_lane_0;
  set_at<0, 32>(debug_compute_result_lane_0, debug_compute_result.extract<0, 31>());
  *global_debug_handle << "out_jacobi2d_compute0," << (1*root + 0) << ", " << d1<< "," << d0<< "," <<  debug_compute_result_lane_0 << endl;
#endif //__VIVADO_SYNTH__
}

// Driver function
void jacobi2d(HWStream<hw_uint<32> >& /* no bundle get_args num ports = 1 */in, HWStream<hw_uint<32> >& /* get_args num ports = 1 */out) {

#ifndef __VIVADO_SYNTH__
  ofstream debug_file("jacobi2d_debug.csv");
  global_debug_handle = &debug_file;
#endif //__VIVADO_SYNTH__
  I_cache I;
#ifdef __VIVADO_SYNTH__
#pragma HLS dependence variable=I inter false
#endif //__VIVADO_SYNTH__
	for (int c0 = 0; c0 <= 31; c0 += 1)
	  for (int c1 = 0; c1 <= 31; c1 += 1) {
	I_id0(in, I, 0, c0, c1);
	    if (c0 >= 2 && c1 >= 1 && c1 <= 30)
	out_jacobi2d_compute0(I, out, 0, c0 - 1, c1);
	  }
	
#ifndef __VIVADO_SYNTH__
  debug_file.close();
#endif //__VIVADO_SYNTH__
}
