...
int main() {
  ...
  pyr_blnd_kernel(out, in, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_pyr_blnd_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in);
  free(out);
}
