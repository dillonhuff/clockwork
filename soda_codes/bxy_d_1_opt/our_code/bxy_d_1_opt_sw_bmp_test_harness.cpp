...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > bxy_d_1_update_0_write_channel;
  // In lanes = 1
  ...
  bxy_d_1_opt(input_update_0_read_channel, bxy_d_1_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 1; cl++) {
      auto packed_val = bxy_d_1_update_0_write_channel.read();
  ...
  output.save_image("./images/bxy_d_1_opt_bmp_out.bmp");
}
