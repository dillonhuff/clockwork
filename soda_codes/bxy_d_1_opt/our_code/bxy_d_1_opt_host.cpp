...
int main(int argc, char **argv) {
  ...
  const int bxy_d_1_update_0_write_pipe0_DATA_SIZE = num_epochs*2073600;
  const int bxy_d_1_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t bxy_d_1_update_0_write_pipe0_size_bytes = bxy_d_1_update_0_write_pipe0_BYTES_PER_PIXEL * bxy_d_1_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += bxy_d_1_update_0_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > bxy_d_1_update_0_write_pipe0(bxy_d_1_update_0_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "bxy_d_1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer bxy_d_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, bxy_d_1_update_0_write_pipe0_size_bytes, bxy_d_1_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, bxy_d_1_update_0_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("bxy_d_1_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < bxy_d_1_update_0_write_pipe0_DATA_SIZE; i++) {
