...
int main() {
  ...
  oned_r_kernel(out, in, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_oned_r_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in);
  free(out);
}
