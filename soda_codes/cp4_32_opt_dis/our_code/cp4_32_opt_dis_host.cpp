...
int main(int argc, char **argv) {
  ...
  const int cp4_32_1_merged2309_write_pipe0_DATA_SIZE = num_epochs*2073600;
  const int cp4_32_1_merged2309_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t cp4_32_1_merged2309_write_pipe0_size_bytes = cp4_32_1_merged2309_write_pipe0_BYTES_PER_PIXEL * cp4_32_1_merged2309_write_pipe0_DATA_SIZE;

  total_size_bytes += cp4_32_1_merged2309_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > cp4_32_1_merged2309_write_pipe0(cp4_32_1_merged2309_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "cp4_32_opt_dis_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer cp4_32_1_merged2309_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, cp4_32_1_merged2309_write_pipe0_size_bytes, cp4_32_1_merged2309_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, cp4_32_1_merged2309_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("cp4_32_1_merged2309_write_pipe0_accel_result.csv");
    for (int i = 0; i < cp4_32_1_merged2309_write_pipe0_DATA_SIZE; i++) {
