...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > input_1_merged132_read_channel;
  HWStream<hw_uint<512> > blurx7_32_1_merged135_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1952 / 32; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        input_1_merged132_read_channel.write(packed);
    }
  }
  blurx7_32_opt(input_1_merged132_read_channel, blurx7_32_1_merged135_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = blurx7_32_1_merged135_write_channel.read();
  ...
  output.save_image("./images/blurx7_32_opt_bmp_out.bmp");
}
