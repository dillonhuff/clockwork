...
int main(int argc, char **argv) {
  ...
  const int sblr30_1_update_0_write_DATA_SIZE = num_epochs*2079604;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "sblr30_1_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, sblr30_1_update_0_write_ocl_buf));
