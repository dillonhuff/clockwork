...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<32> > ef_2_update_0_write_channel;
  // In lanes = 2
  for (int r = 0; r < 1927; r++) {
    for (int cl = 0; cl < 1090 / 2; cl++) {
      hw_uint<32> packed;
      {
      int c = 2*cl + 0;
  ...
  ef_2_opt(in_update_0_read_channel, ef_2_update_0_write_channel);
  bitmap_image output(1080, 1920);
  for (int r = 0; r < 1920; r++) {
    for (int cl = 0; cl < 1080 / 2; cl++) {
      auto packed_val = ef_2_update_0_write_channel.read();
  ...
  output.save_image("./images/ef_2_opt_bmp_out.bmp");
}
