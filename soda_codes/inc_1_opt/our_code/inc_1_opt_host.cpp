...
int main(int argc, char **argv) {
  ...
  const int inc_1_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t inc_1_update_0_write_pipe0_size_bytes = inc_1_update_0_write_pipe0_BYTES_PER_PIXEL * inc_1_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += inc_1_update_0_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > inc_1_update_0_write_pipe0(inc_1_update_0_write_pipe0_size_bytes);
...
//#endif // __POPULATE_HOST_INPUTS__
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "inc_1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer inc_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, inc_1_update_0_write_pipe0_size_bytes, inc_1_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, inc_1_update_0_write_pipe0_ocl_buf));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({inc_1_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
{
    std::ofstream regression_result("inc_1_update_0_write_pipe0_accel_result.csv");
