...
hw_uint<16> inc_1_generated_compute(hw_uint<16*1> in) {
  return uint16_t((uint16_t(in.get<16, 0>()) + 1));
}
...
hw_uint<16>  in_generated_compute_unrolled_1(hw_uint<16>& in_off_chip) {
  ...
  // inc_1_update_0 unroll factor: 1
hw_uint<16>  inc_1_generated_compute_unrolled_1(hw_uint<16>& in) {
  hw_uint<16> whole_result;

  hw_uint<16> lane_0_in;
  set_at<0, 16, 16>(lane_0_in, in.extract<0, 15>());
  auto result_0 = inc_1_generated_compute(lane_0_in);
  set_at<0, 16, 16>(whole_result, result_0);
  return whole_result;
}
