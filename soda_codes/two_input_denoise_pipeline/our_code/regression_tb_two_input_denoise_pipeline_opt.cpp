...
int main() {
  ...
  HWStream<hw_uint<32> > two_input_denoise_pipeline_update_0_write;
  ...
  two_input_denoise_pipeline_opt(f_update_0_read, u_update_0_read, two_input_denoise_pipeline_update_0_write);

  for (int i = 0; i < 900; i++) {
    hw_uint<32> actual = two_input_denoise_pipeline_update_0_write.read();
