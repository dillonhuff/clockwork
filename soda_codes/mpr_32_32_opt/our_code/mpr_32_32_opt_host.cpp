...
int main(int argc, char **argv) {
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > mpr_32_32_update_0_write_pipe0(mpr_32_32_update_0_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "mpr_32_32_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer mpr_32_32_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, mpr_32_32_update_0_write_pipe0_size_bytes, mpr_32_32_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, mpr_32_32_update_0_write_pipe0_ocl_buf));
