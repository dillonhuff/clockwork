...
int main() {
  ofstream in_pix("input_pixels_regression_result_gp64x64_16_opt.txt");
  ofstream fout("regression_result_gp64x64_16_opt.txt");
  HWStream<hw_uint<256> > in_update_0_read;
  HWStream<hw_uint<16> > gp64x64_16_update_0_write;


  // Loading input data
  // cmap    : { in_update_0[root = 0, in_0, in_1] -> in_off_chip[0, 0] : 0 <= in_0 <= 67 and 0 <= in_1 <= 1038 }
  // read map: { in_off_chip[0, 0] -> in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 67 and 0 <= in_1 <= 1038 }
  // rng     : { in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 67 and 0 <= in_1 <= 1038 }
  for (int i = 0; i < 70652; i++) {
    hw_uint<256> in_val;
  ...
  gp64x64_16_opt(in_update_0_read, gp64x64_16_update_0_write);

  for (int i = 0; i < 4096; i++) {
    hw_uint<16> actual = gp64x64_16_update_0_write.read();
