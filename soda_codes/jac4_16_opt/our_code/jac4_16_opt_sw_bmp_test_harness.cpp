...
#include <fstream>
#include <vector>
#include "jac4_16_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<256> > in_1_merged563_read_channel;
  HWStream<hw_uint<256> > jac4_16_1_merged611_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1095; r++) {
    for (int cl = 0; cl < 2160 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        in_1_merged563_read_channel.write(packed);
    }
  }
  jac4_16_opt(in_1_merged563_read_channel, jac4_16_1_merged611_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 16; cl++) {
      auto packed_val = jac4_16_1_merged611_write_channel.read();
  ...
  output.save_image("./images/jac4_16_opt_bmp_out.bmp");
}
