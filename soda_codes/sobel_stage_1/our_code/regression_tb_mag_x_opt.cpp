...
int main() {
  ofstream in_pix("input_pixels_regression_result_mag_x_opt.txt");
  ofstream fout("regression_result_mag_x_opt.txt");
  HWStream<hw_uint<32> > img_update_0_read;
  HWStream<hw_uint<32> > mag_x_update_0_write;
  ...
  mag_x_opt(img_update_0_read, mag_x_update_0_write);

  for (int i = 0; i < 1024; i++) {
    hw_uint<32> actual = mag_x_update_0_write.read();
