...
#include <fstream>
#include <vector>
#include "sc_dyn_60_32.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<512> > pw_math_in_oc02_merged6870_read_channel;
  HWStream<hw_uint<512> > pw_math_stencil_59304306_merged7053_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1200; r++) {
    for (int cl = 0; cl < 2880 / 16; cl++) {
      hw_uint<512> packed;
      {
      int c = 16*cl + 0;
      ...
      if (r < input.height() && c < input.width()) {
        ...
        pw_math_in_oc02_merged6870_read_channel.write(packed);
    }
  }
  sc_dyn_60_32(pw_math_in_oc02_merged6870_read_channel, pw_math_stencil_59304306_merged7053_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 16; cl++) {
      auto packed_val = pw_math_stencil_59304306_merged7053_write_channel.read();
  ...
  output.save_image("./images/sc_dyn_60_32_bmp_out.bmp");
}
