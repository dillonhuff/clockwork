...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > gp_sm_update_0_write_channel;
  // In lanes = 8
  for (int r = 0; r < 519; r++) {
    for (int cl = 0; cl < 536 / 8; cl++) {
      hw_uint<128> packed;
      {
      int c = 8*cl + 0;
  ...
  gp_sm_opt(in_update_0_read_channel, gp_sm_update_0_write_channel);
  bitmap_image output(64, 64);
  for (int r = 0; r < 64; r++) {
    for (int cl = 0; cl < 64 / 1; cl++) {
      auto packed_val = gp_sm_update_0_write_channel.read();
  ...
  output.save_image("./images/gp_sm_opt_bmp_out.bmp");
}
