...
#include <fstream>
#include <vector>
#include "sbl9_32_opt_dis.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<512> > img_1_merged830_read_channel;
  HWStream<hw_uint<512> > sbl9_32_1_merged839_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1082; r++) {
    for (int cl = 0; cl < 1952 / 32; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        img_1_merged830_read_channel.write(packed);
    }
  }
  sbl9_32_opt_dis(img_1_merged830_read_channel, sbl9_32_1_merged839_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = sbl9_32_1_merged839_write_channel.read();
  ...
  output.save_image("./images/sbl9_32_opt_dis_bmp_out.bmp");
}
