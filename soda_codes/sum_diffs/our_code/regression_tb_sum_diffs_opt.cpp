...
int main() {
  ofstream in_pix("input_pixels_regression_result_sum_diffs_opt.txt");
  ofstream fout("regression_result_sum_diffs_opt.txt");
  HWStream<hw_uint<32> > f_update_0_read;
  HWStream<hw_uint<32> > u_update_0_read;
  HWStream<hw_uint<32> > sum_diffs_update_0_write;
  ...
  sum_diffs_opt(f_update_0_read, u_update_0_read, sum_diffs_update_0_write);

  for (int i = 0; i < 900; i++) {
    hw_uint<32> actual = sum_diffs_update_0_write.read();
