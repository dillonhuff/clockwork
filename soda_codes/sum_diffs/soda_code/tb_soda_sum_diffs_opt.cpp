...
int main() {
  ...
  sum_diffs_opt_kernel(sum_diffs, f_off_chip, u_off_chip, transfer_cols);
  write_results<bits_per_pixel>("soda_sum_diffs_opt_regression_result.csv", sum_diffs, nrows, ncols, transfer_cols);
  free(f_off_chip);
  free(u_off_chip);
  free(sum_diffs);
}
