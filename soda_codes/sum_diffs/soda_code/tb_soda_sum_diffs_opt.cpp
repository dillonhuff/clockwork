// AUTO GEN SODA TB
#include "sum_diffs_opt_kernel.h"
#include <iostream>
#include <fstream>
...
int main() {
  ...
  ap_uint<BURST_WIDTH>* sum_diffs = (ap_uint<BURST_WIDTH>*) malloc(sizeof(ap_uint<BURST_WIDTH>)*num_transfers);
  ...
  sum_diffs_opt_kernel(sum_diffs, f_off_chip, u_off_chip, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_sum_diffs_opt_regression_result.csv", sum_diffs, nrows, ncols, transfer_cols);
  free(f_off_chip);
  free(u_off_chip);
  free(sum_diffs);
}
