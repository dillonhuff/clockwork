...
int main(int argc, char **argv) {
  ...
  const int psefn_4_update_0_write_DATA_SIZE = num_epochs*3722964;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "psefn_4_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, psefn_4_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(3722964 / 4);
