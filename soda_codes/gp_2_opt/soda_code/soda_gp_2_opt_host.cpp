...
int main(int argc, char **argv) {
  ...
  const int gp_2_update_0_write_DATA_SIZE = num_epochs*6320;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "gp_2_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, gp_2_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(6320 / 2);
