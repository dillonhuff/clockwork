...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > in_1_merged3370_read_channel;
  HWStream<hw_uint<512> > jac9_32_1_merged3340_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1095; r++) {
    for (int cl = 0; cl < 2400 / 32; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        in_1_merged3370_read_channel.write(packed);
    }
  }
  jac9_32_opt_d32(in_1_merged3370_read_channel, jac9_32_1_merged3340_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = jac9_32_1_merged3340_write_channel.read();
