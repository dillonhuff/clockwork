...
#include <fstream>
#include <vector>
#include "blur4_16_opt_dis.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<256> > input_1_merged257_read_channel;
  HWStream<hw_uint<256> > blur4_16_1_merged263_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1082; r++) {
    for (int cl = 0; cl < 1936 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        input_1_merged257_read_channel.write(packed);
    }
  }
  blur4_16_opt_dis(input_1_merged257_read_channel, blur4_16_1_merged263_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 16; cl++) {
      auto packed_val = blur4_16_1_merged263_write_channel.read();
  ...
  output.save_image("./images/blur4_16_opt_dis_bmp_out.bmp");
}
