...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > dp_update_0_write_channel;
  // In lanes = 1
  for (int r = 0; r < 32; r++) {
  ...
  dp_opt(img_update_0_read_channel, dp_update_0_write_channel);
  bitmap_image output(32, 32);
  for (int r = 0; r < 32; r++) {
    for (int cl = 0; cl < 32 / 1; cl++) {
      auto packed_val = dp_update_0_write_channel.read();
  ...
  output.save_image("./images/dp_opt_bmp_out.bmp");
}
