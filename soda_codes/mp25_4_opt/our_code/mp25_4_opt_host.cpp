...
int main(int argc, char **argv) {
  ...
  const int mp25_4_update_0_write_pipe0_DATA_SIZE = num_epochs*262144;
  const int mp25_4_update_0_write_pipe0_BYTES_PER_PIXEL = 32 / 8;
  size_t mp25_4_update_0_write_pipe0_size_bytes = mp25_4_update_0_write_pipe0_BYTES_PER_PIXEL * mp25_4_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += mp25_4_update_0_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > mp25_4_update_0_write_pipe0(mp25_4_update_0_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "mp25_4_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer mp25_4_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, mp25_4_update_0_write_pipe0_size_bytes, mp25_4_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, mp25_4_update_0_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("mp25_4_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < mp25_4_update_0_write_pipe0_DATA_SIZE; i++) {
      regression_result << ((uint32_t*) (mp25_4_update_0_write_pipe0.data()))[i] << std::endl;
    }
}
