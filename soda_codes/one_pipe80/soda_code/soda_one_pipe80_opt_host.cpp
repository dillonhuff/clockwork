...
int main(int argc, char **argv) {
  ...
  const int in_off_chip0_update_0_read_DATA_SIZE = num_epochs*2073600;
  ...
  const int one_pipe80_update_0_write_DATA_SIZE = num_epochs*2073600;
  const int one_pipe80_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t one_pipe80_update_0_write_size_bytes = one_pipe80_update_0_write_BYTES_PER_PIXEL * one_pipe80_update_0_write_DATA_SIZE;

  total_size_bytes += one_pipe80_update_0_write_size_bytes;
  total_size_bytes_written += one_pipe80_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > one_pipe80_update_0_write(one_pipe80_update_0_write_size_bytes);
  ...
  for (int i = 0; i < one_pipe80_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (one_pipe80_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "one_pipe80_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer one_pipe80_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, one_pipe80_update_0_write_size_bytes, one_pipe80_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, one_pipe80_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(2073600 / 8);
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({one_pipe80_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("one_pipe80_update_0_write_accel_result.csv");
  for (int i = 0; i < one_pipe80_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (one_pipe80_update_0_write.data()))[i] << std::endl;
  }
