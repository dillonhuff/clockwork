...
int main(int argc, char **argv) {
  ...
  const int mpr16b_32_1_update_0_write_DATA_SIZE = num_epochs*1048576;
  const int mpr16b_32_1_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t mpr16b_32_1_update_0_write_size_bytes = mpr16b_32_1_update_0_write_BYTES_PER_PIXEL * mpr16b_32_1_update_0_write_DATA_SIZE;

  total_size_bytes += mpr16b_32_1_update_0_write_size_bytes;
  total_size_bytes_written += mpr16b_32_1_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > mpr16b_32_1_update_0_write(mpr16b_32_1_update_0_write_size_bytes);
  ...
  for (int i = 0; i < mpr16b_32_1_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (mpr16b_32_1_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "mpr16b_32_1_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer mpr16b_32_1_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, mpr16b_32_1_update_0_write_size_bytes, mpr16b_32_1_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, mpr16b_32_1_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(1048576 / 1);
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({mpr16b_32_1_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("mpr16b_32_1_update_0_write_accel_result.csv");
  for (int i = 0; i < mpr16b_32_1_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (mpr16b_32_1_update_0_write.data()))[i] << std::endl;
  }
