...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<32> > dummy_app_update_0_write_channel;
  // In lanes = 1
  for (int r = 0; r < 31; r++) {
    for (int cl = 0; cl < 30 / 1; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        u_update_0_read_channel.write(packed);
    }
  }
  ...
  for (int r = 0; r < 30; r++) {
    for (int cl = 0; cl < 30 / 1; cl++) {
      auto packed_val = dummy_app_update_0_write_channel.read();
  ...
  output.save_image("./images/dummy_app_opt_bmp_out.bmp");
}
