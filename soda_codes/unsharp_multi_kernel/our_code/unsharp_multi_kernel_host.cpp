...
int main(int argc, char **argv) {
  ...
  total_size_bytes += diff_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > diff_write_pipe0(diff_write_pipe0_size_bytes);
  std::vector<uint8_t, aligned_allocator<uint8_t> > oc_load_in03_read_pipe0(oc_load_in03_read_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "unsharp_multi_kernel_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer oc_load_in03_read_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, oc_load_in03_read_pipe0_size_bytes, oc_load_in03_read_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, oc_load_in03_read_pipe0_ocl_buf));

  OCL_CHECK(err, cl::Buffer diff_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, diff_write_pipe0_size_bytes, diff_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, diff_write_pipe0_ocl_buf));
