...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > gp_fpga_update_0_write_channel;
  // In lanes = 8
  ...
  gp_fpga_naive(in_update_0_read_channel, gp_fpga_update_0_write_channel);
  bitmap_image output(240, 135);
  for (int r = 0; r < 135; r++) {
    for (int cl = 0; cl < 240 / 1; cl++) {
      auto packed_val = gp_fpga_update_0_write_channel.read();
  ...
  output.save_image("./images/gp_fpga_naive_bmp_out.bmp");
}
