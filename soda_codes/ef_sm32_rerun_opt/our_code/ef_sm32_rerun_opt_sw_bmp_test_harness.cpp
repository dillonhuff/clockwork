...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > ef_sm32_rerun_update_0_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1927; r++) {
    for (int cl = 0; cl < 1280 / 32; cl++) {
      hw_uint<512> packed;
      {
      int c = 32*cl + 0;
  ...
  ef_sm32_rerun_opt(in_update_0_read_channel, ef_sm32_rerun_update_0_write_channel);
  bitmap_image output(1088, 1920);
  for (int r = 0; r < 1920; r++) {
    for (int cl = 0; cl < 1088 / 32; cl++) {
      auto packed_val = ef_sm32_rerun_update_0_write_channel.read();
  ...
  output.save_image("./images/ef_sm32_rerun_opt_bmp_out.bmp");
}
