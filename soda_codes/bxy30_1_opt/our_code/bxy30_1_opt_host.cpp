...
int main(int argc, char **argv) {
  ...
  const int bxy30_1_update_0_write_DATA_SIZE = num_epochs*2073600;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "bxy30_1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, bxy30_1_update_0_write_ocl_buf));
