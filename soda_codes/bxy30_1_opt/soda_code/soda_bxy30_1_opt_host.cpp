...
int main(int argc, char **argv) {
  ...
  const int bxy30_1_update_0_write_DATA_SIZE = num_epochs*2079604;
  const int bxy30_1_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t bxy30_1_update_0_write_size_bytes = bxy30_1_update_0_write_BYTES_PER_PIXEL * bxy30_1_update_0_write_DATA_SIZE;

  total_size_bytes += bxy30_1_update_0_write_size_bytes;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "bxy30_1_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer bxy30_1_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, bxy30_1_update_0_write_size_bytes, bxy30_1_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, bxy30_1_update_0_write_ocl_buf));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({bxy30_1_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
