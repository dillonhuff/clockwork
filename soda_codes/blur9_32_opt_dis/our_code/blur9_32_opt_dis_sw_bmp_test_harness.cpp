...
#include <fstream>
#include <vector>
#include "blur9_32_opt_dis.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<512> > input_1_merged481_read_channel;
  HWStream<hw_uint<512> > blur9_32_1_merged487_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1082; r++) {
    for (int cl = 0; cl < 1952 / 32; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        input_1_merged481_read_channel.write(packed);
    }
  }
  blur9_32_opt_dis(input_1_merged481_read_channel, blur9_32_1_merged487_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = blur9_32_1_merged487_write_channel.read();
  ...
  output.save_image("./images/blur9_32_opt_dis_bmp_out.bmp");
}
