...
int main(int argc, char **argv) {
  ...
  const int bxy_ur_32_update_0_write_DATA_SIZE = 2112064;
  const int bxy_ur_32_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t bxy_ur_32_update_0_write_size_bytes = bxy_ur_32_update_0_write_BYTES_PER_PIXEL * bxy_ur_32_update_0_write_DATA_SIZE;

  const int input_update_0_read_DATA_SIZE = 2112064;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > bxy_ur_32_update_0_write(bxy_ur_32_update_0_write_size_bytes);
  ...
  for (int i = 0; i < bxy_ur_32_update_0_write_DATA_SIZE; i++) {
    // TODO: Add support for other widths
    ((uint16_t*) (bxy_ur_32_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "bxy_ur_32_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer bxy_ur_32_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, bxy_ur_32_update_0_write_size_bytes, bxy_ur_32_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, bxy_ur_32_update_0_write_ocl_buf));
  ...
  std::ofstream regression_result("bxy_ur_32_update_0_write_accel_result.csv");
  for (int i = 0; i < bxy_ur_32_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (bxy_ur_32_update_0_write.data()))[i] << std::endl;;
  }
