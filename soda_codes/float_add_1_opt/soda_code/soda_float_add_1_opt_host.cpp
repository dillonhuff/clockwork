...
int main(int argc, char **argv) {
  ...
  const int float_add_1_update_0_write_pipe0_DATA_SIZE = num_epochs*1024;
  const int float_add_1_update_0_write_BYTES_PER_PIXEL = 32 / 8;
  size_t float_add_1_update_0_write_size_bytes = float_add_1_update_0_write_BYTES_PER_PIXEL * float_add_1_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += float_add_1_update_0_write_size_bytes;
  total_size_bytes_written += float_add_1_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > float_add_1_update_0_write_pipe0(float_add_1_update_0_write_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "float_add_1_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer float_add_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, float_add_1_update_0_write_size_bytes, float_add_1_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, float_add_1_update_0_write_pipe0_ocl_buf));
