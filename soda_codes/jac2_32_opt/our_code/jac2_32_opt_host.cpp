...
int main(int argc, char **argv) {
  ...
  const int in_update_0_read_pipe0_DATA_SIZE = num_epochs*1215450;
  ...
  const int jac2_32_update_0_write_pipe0_DATA_SIZE = num_epochs*1166400;
  const int jac2_32_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t jac2_32_update_0_write_pipe0_size_bytes = jac2_32_update_0_write_pipe0_BYTES_PER_PIXEL * jac2_32_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += jac2_32_update_0_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > jac2_32_update_0_write_pipe0(jac2_32_update_0_write_pipe0_size_bytes);
  ...
  for (int i = 0; i < jac2_32_update_0_write_pipe0_DATA_SIZE; i++) {
    ((uint16_t*) (jac2_32_update_0_write_pipe0.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "jac2_32_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer jac2_32_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, jac2_32_update_0_write_pipe0_size_bytes, jac2_32_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, jac2_32_update_0_write_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({jac2_32_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
{
    std::ofstream regression_result("jac2_32_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < jac2_32_update_0_write_pipe0_DATA_SIZE; i++) {
      regression_result << ((uint16_t*) (jac2_32_update_0_write_pipe0.data()))[i] << std::endl;
    }
}
