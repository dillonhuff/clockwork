...
int main(int argc, char **argv) {
  ...
  const int out1_update_0_write_DATA_SIZE = num_epochs*2088960;
  const int out1_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t out1_update_0_write_size_bytes = out1_update_0_write_BYTES_PER_PIXEL * out1_update_0_write_DATA_SIZE;

  total_size_bytes += out1_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > out1_update_0_write(out1_update_0_write_size_bytes);
  ...
  for (int i = 0; i < out1_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (out1_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "out0_out1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, out0_update_0_write_ocl_buf));

  OCL_CHECK(err, cl::Buffer out1_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, out1_update_0_write_size_bytes, out1_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(3, out1_update_0_write_ocl_buf));
...
nsduration = end - start;
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({out1_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
