...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<128> > ef_fpga_8_update_0_write_channel;
  // In lanes = 8
  ...
  ef_fpga_8_opt(in_update_0_read_channel, ef_fpga_8_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 8; cl++) {
      auto packed_val = ef_fpga_8_update_0_write_channel.read();
  ...
  output.save_image("./images/ef_fpga_8_opt_bmp_out.bmp");
}
