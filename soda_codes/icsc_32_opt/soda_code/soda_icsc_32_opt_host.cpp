...
int main(int argc, char **argv) {
  ...
  const int icsc_32_update_0_write_DATA_SIZE = num_epochs*2790400;
  const int icsc_32_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t icsc_32_update_0_write_size_bytes = icsc_32_update_0_write_BYTES_PER_PIXEL * icsc_32_update_0_write_DATA_SIZE;

  total_size_bytes += icsc_32_update_0_write_size_bytes;
  total_size_bytes_written += icsc_32_update_0_write_size_bytes;
  const int in_update_0_read_DATA_SIZE = num_epochs*2790400;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > icsc_32_update_0_write(icsc_32_update_0_write_size_bytes);
  ...
  for (int i = 0; i < icsc_32_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (icsc_32_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "icsc_32_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({icsc_32_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("icsc_32_update_0_write_accel_result.csv");
  for (int i = 0; i < icsc_32_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (icsc_32_update_0_write.data()))[i] << std::endl;
  }
