...
int main() {
  ofstream in_pix("input_pixels_regression_result_hr_8_opt.txt");
  ofstream fout("regression_result_hr_8_opt.txt");
  HWStream<hw_uint<256> > img_update_0_read;
  HWStream<hw_uint<256> > hr_8_update_0_write;


  // Loading input data
  // cmap    : { img_update_0[root = 0, img_0, img_1] -> img_oc[0, 0] : -2 <= img_0 <= 241 and -2 <= img_1 <= 1081 }
  // read map: { img_oc[0, 0] -> img_update_0[root = 0, img_0, img_1] : -2 <= img_0 <= 241 and -2 <= img_1 <= 1081 }
  // rng     : { img_update_0[root = 0, img_0, img_1] : -2 <= img_0 <= 241 and -2 <= img_1 <= 1081 }
  for (int i = 0; i < 264496; i++) {
    hw_uint<256> in_val;
  ...
  hr_8_opt(img_update_0_read, hr_8_update_0_write);

  for (int i = 0; i < 259200; i++) {
    hw_uint<256> actual = hr_8_update_0_write.read();
