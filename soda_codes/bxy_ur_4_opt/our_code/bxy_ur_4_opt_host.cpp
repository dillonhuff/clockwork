...
int main(int argc, char **argv) {
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > bxy_ur_4_update_0_write(bxy_ur_4_update_0_write_size_bytes);
  ...
  // TODO: Add support for other widths
    ((uint16_t*) (bxy_ur_4_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "bxy_ur_4_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer bxy_ur_4_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, bxy_ur_4_update_0_write_size_bytes, bxy_ur_4_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, bxy_ur_4_update_0_write_ocl_buf));

  int bxy_ur_4_update_0_write_size = bxy_ur_4_update_0_write_DATA_SIZE;
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, bxy_ur_4_update_0_write_size));

  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({input_update_0_read_ocl_buf}, 0));

  OCL_CHECK(err, err = q.enqueueTask(krnl_vector_add));

  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({bxy_ur_4_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
