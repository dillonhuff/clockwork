...
int main() {
  ofstream in_pix("input_pixels_regression_result_bxy_ur_4_opt.txt");
  ofstream fout("regression_result_bxy_ur_4_opt.txt");
  HWStream<hw_uint<64> > input_update_0_read;
  HWStream<hw_uint<64> > bxy_ur_4_update_0_write;
  ...
  bxy_ur_4_opt(input_update_0_read, bxy_ur_4_update_0_write);

  for (int i = 0; i < 518400; i++) {
    hw_uint<64> actual = bxy_ur_4_update_0_write.read();
