...
int main(int argc, char **argv) {
  ...
  const int icsc_nd_32_update_0_write_DATA_SIZE = num_epochs*2790400;
  const int icsc_nd_32_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t icsc_nd_32_update_0_write_size_bytes = icsc_nd_32_update_0_write_BYTES_PER_PIXEL * icsc_nd_32_update_0_write_DATA_SIZE;

  total_size_bytes += icsc_nd_32_update_0_write_size_bytes;
  total_size_bytes_written += icsc_nd_32_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > icsc_nd_32_update_0_write(icsc_nd_32_update_0_write_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "icsc_nd_32_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer icsc_nd_32_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, icsc_nd_32_update_0_write_size_bytes, icsc_nd_32_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, icsc_nd_32_update_0_write_pipe0_ocl_buf));
...
nsduration = end - start;
  ...
  std::ofstream regression_result("icsc_nd_32_update_0_write_accel_result.csv");
  for (int i = 0; i < icsc_nd_32_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (icsc_nd_32_update_0_write.data()))[i] << std::endl;
  }
