...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > psef_ct2_32_update_0_write_channel;
  ...
  psef_ct2_32_opt(in_update_0_read_channel, psef_ct2_32_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int c = 0; c < 1920; c++) {
      auto val = psef_ct2_32_update_0_write_channel.read();
      rgb_t pix;
      pix.red = val;
      pix.green = val;
      pix.blue = val;
  ...
  output.save_image("./images/psef_ct2_32_opt_bmp_out.bmp");
}
