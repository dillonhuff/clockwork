...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > ef_sm1_update_0_write_channel;
  // In lanes = 1
  ...
  ef_sm1_opt(in_update_0_read_channel, ef_sm1_update_0_write_channel);
  bitmap_image output(1080, 1920);
  for (int r = 0; r < 1920; r++) {
    for (int cl = 0; cl < 1080 / 1; cl++) {
      auto packed_val = ef_sm1_update_0_write_channel.read();
  ...
  output.save_image("./images/ef_sm1_opt_bmp_out.bmp");
}
