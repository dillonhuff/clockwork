...
int main(int argc, char **argv) {
  ...
  const int mp25_8_update_0_write_DATA_SIZE = num_epochs*1048576;
  const int mp25_8_update_0_write_BYTES_PER_PIXEL = 32 / 8;
  size_t mp25_8_update_0_write_size_bytes = mp25_8_update_0_write_BYTES_PER_PIXEL * mp25_8_update_0_write_DATA_SIZE;

  total_size_bytes += mp25_8_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > mp25_8_update_0_write(mp25_8_update_0_write_size_bytes);
  ...
  for (int i = 0; i < mp25_8_update_0_write_DATA_SIZE; i++) {
    ((uint32_t*) (mp25_8_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "mp25_8_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer mp25_8_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, mp25_8_update_0_write_size_bytes, mp25_8_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, mp25_8_update_0_write_ocl_buf));
...
nsduration = end - start;
  ...
  std::ofstream regression_result("mp25_8_update_0_write_accel_result.csv");
  for (int i = 0; i < mp25_8_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint32_t*) (mp25_8_update_0_write.data()))[i] << std::endl;
  }
