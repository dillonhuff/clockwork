...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > img_1_merged814_read_channel;
  HWStream<hw_uint<512> > sbl_dlb_32_1_merged823_write_channel;
  // In lanes = 32
  for (int r = 0; r < 4098; r++) {
    for (int cl = 0; cl < 4128 / 32; cl++) {
      hw_uint<512> packed;
      {
      int c = 32*cl + 0;
      ...
      if (r < input.height() && c < input.width()) {
        ...
        img_1_merged814_read_channel.write(packed);
    }
  }
  sbl_dlb_32_opt(img_1_merged814_read_channel, sbl_dlb_32_1_merged823_write_channel);
  bitmap_image output(4096, 4096);
  for (int r = 0; r < 4096; r++) {
    for (int cl = 0; cl < 4096 / 32; cl++) {
      auto packed_val = sbl_dlb_32_1_merged823_write_channel.read();
  ...
  output.save_image("./images/sbl_dlb_32_opt_bmp_out.bmp");
}
