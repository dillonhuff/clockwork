...
int main(int argc, char **argv) {
  ...
  const int pwls_ur_16_update_0_write_DATA_SIZE = 2073600;
  const int pwls_ur_16_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t pwls_ur_16_update_0_write_size_bytes = pwls_ur_16_update_0_write_BYTES_PER_PIXEL * pwls_ur_16_update_0_write_DATA_SIZE;

  total_size_bytes += pwls_ur_16_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > pwls_ur_16_update_0_write(pwls_ur_16_update_0_write_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "pwls_ur_16_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer pwls_ur_16_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, pwls_ur_16_update_0_write_size_bytes, pwls_ur_16_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, pwls_ur_16_update_0_write_ocl_buf));

  int pwls_ur_16_update_0_write_size = pwls_ur_16_update_0_write_DATA_SIZE;
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, pwls_ur_16_update_0_write_size));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({pwls_ur_16_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
