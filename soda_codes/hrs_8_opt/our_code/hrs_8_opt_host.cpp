...
int main(int argc, char **argv) {
  ...
  const int hrs_8_update_0_write_DATA_SIZE = 2073600;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > hrs_8_update_0_write(hrs_8_update_0_write_size_bytes);
  ...
  for (int i = 0; i < hrs_8_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (hrs_8_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "hrs_8_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer hrs_8_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, hrs_8_update_0_write_size_bytes, hrs_8_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, hrs_8_update_0_write_ocl_buf));

  int hrs_8_update_0_write_size = hrs_8_update_0_write_DATA_SIZE;
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, hrs_8_update_0_write_size));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({hrs_8_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
