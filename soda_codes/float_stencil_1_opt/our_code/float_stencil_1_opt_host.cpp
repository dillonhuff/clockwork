...
int main(int argc, char **argv) {
  ...
  const int float_stencil_1_update_0_write_pipe0_BYTES_PER_PIXEL = 32 / 8;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > float_stencil_1_update_0_write_pipe0(float_stencil_1_update_0_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "float_stencil_1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer float_stencil_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, float_stencil_1_update_0_write_pipe0_size_bytes, float_stencil_1_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, float_stencil_1_update_0_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("float_stencil_1_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < float_stencil_1_update_0_write_pipe0_DATA_SIZE; i++) {
