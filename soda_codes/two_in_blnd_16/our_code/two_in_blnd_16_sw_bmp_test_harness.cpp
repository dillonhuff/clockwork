...
#include <fstream>
#include <vector>
#include "two_in_blnd_16.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<512> > pw_math_in0_oc02_merged894_read_channel;
  HWStream<hw_uint<512> > pw_math_in1_oc46_merged897_read_channel;
  HWStream<hw_uint<512> > pw_math_merged_0_reconstruct_lp138_buf141147149_merged975_write_channel;
  // In lanes = 16
  for (int r = 0; r < 2055; r++) {
    for (int cl = 0; cl < 2160 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        pw_math_in0_oc02_merged894_read_channel.write(packed);
    }
  }
  // In lanes = 16
  for (int r = 0; r < 2055; r++) {
    for (int cl = 0; cl < 2160 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        pw_math_in1_oc46_merged897_read_channel.write(packed);
    }
  }
  two_in_blnd_16(pw_math_in0_oc02_merged894_read_channel, pw_math_in1_oc46_merged897_read_channel, pw_math_merged_0_reconstruct_lp138_buf141147149_merged975_write_channel);
  bitmap_image output(2048, 2048);
  for (int r = 0; r < 2048; r++) {
    for (int cl = 0; cl < 2048 / 16; cl++) {
      auto packed_val = pw_math_merged_0_reconstruct_lp138_buf141147149_merged975_write_channel.read();
  ...
  output.save_image("./images/two_in_blnd_16_bmp_out.bmp");
}
