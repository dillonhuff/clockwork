...
int main(int argc, char **argv) {
  ...
  total_size_bytes += input_update_0_sm16_029_read_pipe0_size_bytes;
  ...
  std::ofstream input_input_update_0_sm16_029_read("input_update_0_sm16_029_read.csv");
  for (int i = 0; i < input_update_0_sm16_029_read_pipe0_DATA_SIZE; i++) {
...
#ifdef __FLOAT_OUTPUT__
    input_input_update_0_sm16_029_read << val << std::endl;
#else // __FLOAT_OUTPUT__
    input_input_update_0_sm16_029_read << val << std::endl;
#endif // __FLOAT_OUTPUT__

#ifdef __FLOAT_OUTPUT__
    ((uint16_t*) (input_update_0_sm16_029_read_pipe0.data()))[i] = bitcast<uint16_t, float>(val);
#else // __FLOAT_OUTPUT__
    ((uint16_t*) (input_update_0_sm16_029_read_pipe0.data()))[i] = val;
#endif // __FLOAT_OUTPUT__
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "bxy2_d_1_opt_d_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({input_update_0_sm16_029_read_pipe0_ocl_buf}, 0));
