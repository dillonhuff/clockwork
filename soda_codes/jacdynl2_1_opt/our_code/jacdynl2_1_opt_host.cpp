...
int main(int argc, char **argv) {
  ...
  const int jacdynl2_1_update_0_write_pipe0_DATA_SIZE = num_epochs*1166400;
  const int jacdynl2_1_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t jacdynl2_1_update_0_write_pipe0_size_bytes = jacdynl2_1_update_0_write_pipe0_BYTES_PER_PIXEL * jacdynl2_1_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += jacdynl2_1_update_0_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > jacdynl2_1_update_0_write_pipe0(jacdynl2_1_update_0_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "jacdynl2_1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer jacdynl2_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, jacdynl2_1_update_0_write_pipe0_size_bytes, jacdynl2_1_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, jacdynl2_1_update_0_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("jacdynl2_1_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < jacdynl2_1_update_0_write_pipe0_DATA_SIZE; i++) {
