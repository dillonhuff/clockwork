...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > jacdynl2_1_update_0_write_channel;
  // In lanes = 1
  for (int r = 0; r < 1095; r++) {
    for (int cl = 0; cl < 1110 / 1; cl++) {
      hw_uint<16> packed;
      {
      int c = 1*cl + 0;
  ...
  jacdynl2_1_opt(in_update_0_read_channel, jacdynl2_1_update_0_write_channel);
  bitmap_image output(1080, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1080 / 1; cl++) {
      auto packed_val = jacdynl2_1_update_0_write_channel.read();
  ...
  output.save_image("./images/jacdynl2_1_opt_bmp_out.bmp");
}
