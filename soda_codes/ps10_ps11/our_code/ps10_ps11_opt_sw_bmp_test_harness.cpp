...
#include <fstream>
#include <vector>
#include "ps10_ps11_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<16> > in_off_chip0_update_0_read_channel;
  HWStream<hw_uint<16> > in_off_chip1_update_0_read_channel;
  HWStream<hw_uint<16> > ps10_update_0_write_channel;
  HWStream<hw_uint<16> > ps11_update_0_write_channel;
  // In lanes = 1
  for (int r = 0; r < 1087; r++) {
    for (int cl = 0; cl < 964 / 1; cl++) {
      hw_uint<16> packed;
      {
      int c = 1*cl + 0;
  ...
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 960 / 1; cl++) {
      hw_uint<16> packed;
      auto packed_val = ps10_update_0_write_channel.read();
  ...
  output.save_image("./images/ps10_ps11_opt_bmp_out.bmp");
}
