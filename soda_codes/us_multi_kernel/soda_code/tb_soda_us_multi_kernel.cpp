// AUTO GEN SODA TB
#include "us_multi_kernel_kernel.h"
#include <iostream>
#include <fstream>
...
int main() {
  ...
  us_multi_kernel_kernel(out, in, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_us_multi_kernel_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in);
  free(out);
}
