...
#include <fstream>
#include <vector>
#include "jacdyn_16_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<256> > in_1_merged1793_read_channel;
  HWStream<hw_uint<256> > jacdyn_16_1_merged1841_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1950; r++) {
    for (int cl = 0; cl < 1328 / 16; cl++) {
      hw_uint<256> packed;
      {
      int c = 16*cl + 0;
      ...
      if (r < input.height() && c < input.width()) {
        ...
        in_1_merged1793_read_channel.write(packed);
    }
  }
  jacdyn_16_opt(in_1_merged1793_read_channel, jacdyn_16_1_merged1841_write_channel);
  bitmap_image output(1088, 1920);
  for (int r = 0; r < 1920; r++) {
    for (int cl = 0; cl < 1088 / 16; cl++) {
      auto packed_val = jacdyn_16_1_merged1841_write_channel.read();
  ...
  output.save_image("./images/jacdyn_16_opt_bmp_out.bmp");
}
