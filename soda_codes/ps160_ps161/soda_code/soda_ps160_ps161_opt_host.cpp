...
int main(int argc, char **argv) {
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "ps160_ps161_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, ps160_update_0_write_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, ps161_update_0_write_ocl_buf));
