...
#include <fstream>
#include <vector>
#include "ps160_ps161_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<256> > in_off_chip0_update_0_read_channel;
  HWStream<hw_uint<256> > in_off_chip1_update_0_read_channel;
  HWStream<hw_uint<256> > ps160_update_0_write_channel;
  HWStream<hw_uint<256> > ps161_update_0_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1087; r++) {
    for (int cl = 0; cl < 1056 / 16; cl++) {
      hw_uint<256> packed;
      {
      int c = 16*cl + 0;
  ...
  ps160_ps161_opt(in_off_chip0_update_0_read_channel, in_off_chip1_update_0_read_channel, ps160_update_0_write_channel, ps161_update_0_write_channel);
  bitmap_image output(960, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 960 / 16; cl++) {
      auto packed_val = ps160_update_0_write_channel.read();
  ...
  output.save_image("./images/ps160_ps161_opt_bmp_out.bmp");
}
