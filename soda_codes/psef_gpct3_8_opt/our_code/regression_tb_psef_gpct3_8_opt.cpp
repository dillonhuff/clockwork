#include <fstream>
#include "psef_gpct3_8_opt.h"

int main() {
  ofstream in_pix("input_pixels_regression_result_psef_gpct3_8_opt.txt");
  ofstream fout("regression_result_psef_gpct3_8_opt.txt");
  HWStream<hw_uint<128> > in_update_0_read;
  HWStream<hw_uint<128> > psef_gpct3_8_update_0_write;


  // Loading input data
  // cmap    : { in_update_0[root = 0, in_0, in_1] -> in_off_chip[0, 0] : 0 <= in_0 <= 242 and 0 <= in_1 <= 1086 }
  // read map: { in_off_chip[0, 0] -> in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 242 and 0 <= in_1 <= 1086 }
  // rng     : { in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 242 and 0 <= in_1 <= 1086 }
  for (int i = 0; i < 264141; i++) {
    hw_uint<128> in_val;
  ...
  psef_gpct3_8_opt(in_update_0_read, psef_gpct3_8_update_0_write);

  for (int i = 0; i < 259200; i++) {
    hw_uint<128> actual = psef_gpct3_8_update_0_write.read();
