// AUTO GEN SODA TB
#include "stencil_chain_dynamic_schedule_kernel.h"
#include <iostream>
#include <fstream>
...
int main() {
  ...
  stencil_chain_dynamic_schedule_kernel(out, in_oc, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_stencil_chain_dynamic_schedule_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in_oc);
  free(out);
}
