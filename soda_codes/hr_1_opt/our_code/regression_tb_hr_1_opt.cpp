...
int main() {
  ofstream in_pix("input_pixels_regression_result_hr_1_opt.txt");
  ofstream fout("regression_result_hr_1_opt.txt");
  HWStream<hw_uint<32> > img_update_0_read;
  HWStream<hw_uint<32> > hr_1_update_0_write;
  ...
  hr_1_opt(img_update_0_read, hr_1_update_0_write);

  for (int i = 0; i < 2073600; i++) {
    hw_uint<32> actual = hr_1_update_0_write.read();
