...
int main(int argc, char **argv) {
  ...
  const int gp_1_update_0_write_DATA_SIZE = num_epochs*6241;
  const int gp_1_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t gp_1_update_0_write_size_bytes = gp_1_update_0_write_BYTES_PER_PIXEL * gp_1_update_0_write_DATA_SIZE;

  total_size_bytes += gp_1_update_0_write_size_bytes;
  const int in_update_0_read_DATA_SIZE = num_epochs*6241;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > gp_1_update_0_write(gp_1_update_0_write_size_bytes);
  ...
  for (int i = 0; i < gp_1_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (gp_1_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "gp_1_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer gp_1_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, gp_1_update_0_write_size_bytes, gp_1_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, gp_1_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(6241 / 1);
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({gp_1_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
printf("Execution time = %f (sec) \n", dsduration);
  std::ofstream regression_result("gp_1_update_0_write_accel_result.csv");
  for (int i = 0; i < gp_1_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (gp_1_update_0_write.data()))[i] << std::endl;
  }
