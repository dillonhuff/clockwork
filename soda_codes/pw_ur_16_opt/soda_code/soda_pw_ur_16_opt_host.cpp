...
int main(int argc, char **argv) {
  ...
  const int input_update_0_read_BYTES_PER_PIXEL = 32 / 8;
  ...
  for (int i = 0; i < pw_ur_16_update_0_write_DATA_SIZE; i++) {
    // TODO: Add support for other widths
    ((uint16_t*) (pw_ur_16_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "pw_ur_16_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer pw_ur_16_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, pw_ur_16_update_0_write_size_bytes, pw_ur_16_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, pw_ur_16_update_0_write_ocl_buf));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({pw_ur_16_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));

  q.finish();

  std::ofstream regression_result("pw_ur_16_update_0_write_accel_result.csv");
  for (int i = 0; i < pw_ur_16_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (pw_ur_16_update_0_write.data()))[i] << std::endl;;
  }
