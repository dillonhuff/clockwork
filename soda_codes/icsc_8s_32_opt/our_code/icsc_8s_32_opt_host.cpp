...
int main(int argc, char **argv) {
  ...
  const int icsc_8s_32_update_0_write_pipe0_DATA_SIZE = num_epochs*2073600;
  const int icsc_8s_32_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t icsc_8s_32_update_0_write_pipe0_size_bytes = icsc_8s_32_update_0_write_pipe0_BYTES_PER_PIXEL * icsc_8s_32_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += icsc_8s_32_update_0_write_pipe0_size_bytes;
  const int in_update_0_read_pipe0_DATA_SIZE = num_epochs*2646016;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > icsc_8s_32_update_0_write_pipe0(icsc_8s_32_update_0_write_pipe0_size_bytes);
  ...
  for (int i = 0; i < icsc_8s_32_update_0_write_pipe0_DATA_SIZE; i++) {
    ((uint16_t*) (icsc_8s_32_update_0_write_pipe0.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "icsc_8s_32_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer icsc_8s_32_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, icsc_8s_32_update_0_write_pipe0_size_bytes, icsc_8s_32_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, icsc_8s_32_update_0_write_pipe0_ocl_buf));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({icsc_8s_32_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
{
    std::ofstream regression_result("icsc_8s_32_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < icsc_8s_32_update_0_write_pipe0_DATA_SIZE; i++) {
      regression_result << ((uint16_t*) (icsc_8s_32_update_0_write_pipe0.data()))[i] << std::endl;
    }
}
