...
int main(int argc, char **argv) {
  ...
  const int op_hcompute_hw_output_stencil_write_pipe0_DATA_SIZE = num_epochs*961;
  ...
  const int op_hcompute_hw_input_stencil_read_pipe0_DATA_SIZE = num_epochs*3969;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "strided_conv_accel", &err));
      valid_device++;
      break;
    }
  }
