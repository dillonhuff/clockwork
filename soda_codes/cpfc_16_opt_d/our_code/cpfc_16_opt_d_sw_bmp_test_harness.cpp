...
#include <fstream>
#include <vector>
#include "cpfc_16_opt_d.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<256> > raw_1_merged1179_read_channel;
  HWStream<hw_uint<256> > cpfc_16_1_merged1209_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 1136 / 16; cl++) {
      hw_uint<256> packed;
      {
      int c = 16*cl + 0;
      ...
      if (r < input.height() && c < input.width()) {
        ...
        raw_1_merged1179_read_channel.write(packed);
    }
  }
  cpfc_16_opt_d(raw_1_merged1179_read_channel, cpfc_16_1_merged1209_write_channel);
  bitmap_image output(1088, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1088 / 16; cl++) {
      auto packed_val = cpfc_16_1_merged1209_write_channel.read();
  ...
  output.save_image("./images/cpfc_16_opt_d_bmp_out.bmp");
}
