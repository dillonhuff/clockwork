...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<256> > raw_1_merged1185_read_channel;
  HWStream<hw_uint<256> > cpfc_16_1_merged1215_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 1136 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        raw_1_merged1185_read_channel.write(packed);
    }
  }
  cpfc_16_opt_d(raw_1_merged1185_read_channel, cpfc_16_1_merged1215_write_channel);
  bitmap_image output(1088, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1088 / 16; cl++) {
      auto packed_val = cpfc_16_1_merged1215_write_channel.read();
