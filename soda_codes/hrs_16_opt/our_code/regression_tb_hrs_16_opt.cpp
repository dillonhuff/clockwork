...
int main() {
  ofstream in_pix("input_pixels_regression_result_hrs_16_opt.txt");
  ofstream fout("regression_result_hrs_16_opt.txt");
  HWStream<hw_uint<256> > img_update_0_read;
  HWStream<hw_uint<256> > hrs_16_update_0_write;


  // Loading input data
  // cmap    : { img_update_0[root = 0, img_0, img_1] -> img_oc[0, 0] : 0 <= img_0 <= 119 and 0 <= img_1 <= 1079 }
  // read map: { img_oc[0, 0] -> img_update_0[root = 0, img_0, img_1] : 0 <= img_0 <= 119 and 0 <= img_1 <= 1079 }
  // rng     : { img_update_0[root = 0, img_0, img_1] : 0 <= img_0 <= 119 and 0 <= img_1 <= 1079 }
  ...
  hrs_16_opt(img_update_0_read, hrs_16_update_0_write);

  for (int i = 0; i < 129600; i++) {
    hw_uint<256> actual = hrs_16_update_0_write.read();
