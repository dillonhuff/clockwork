...
int main(int argc, char **argv) {
  ...
  const int cp_1_update_0_write_DATA_SIZE = num_epochs*2073600;
  const int cp_1_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t cp_1_update_0_write_size_bytes = cp_1_update_0_write_BYTES_PER_PIXEL * cp_1_update_0_write_DATA_SIZE;

  total_size_bytes += cp_1_update_0_write_size_bytes;
  const int raw_update_0_read_DATA_SIZE = num_epochs*2085616;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > cp_1_update_0_write(cp_1_update_0_write_size_bytes);
  ...
  for (int i = 0; i < cp_1_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (cp_1_update_0_write.data()))[i] = 0;
  }
  ...
  OCL_CHECK(err, cl::Buffer cp_1_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, cp_1_update_0_write_size_bytes, cp_1_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, cp_1_update_0_write_ocl_buf));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({cp_1_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
printf("Execution time = %f (sec) \n", dsduration);
  std::ofstream regression_result("cp_1_update_0_write_accel_result.csv");
  for (int i = 0; i < cp_1_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (cp_1_update_0_write.data()))[i] << std::endl;
  }
