...
int main() {
  ...
  art_dead_kernel(out, in, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_art_dead_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in);
  free(out);
}
