...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<32> > pw_math_lp_in_on_chip_0_buf32_reconstruct_lp45_buf485457_write_channel;
  // In lanes = 1
  for (int r = 0; r < 67; r++) {
    for (int cl = 0; cl < 67 / 1; cl++) {
      hw_uint<32> packed;
      {
      int c = 1*cl + 0;
  ...
  pyr_blndd2048_ii1(pw_math_in03_read_channel, pw_math_lp_in_on_chip_0_buf32_reconstruct_lp45_buf485457_write_channel);
  bitmap_image output(64, 64);
  for (int r = 0; r < 64; r++) {
    for (int cl = 0; cl < 64 / 1; cl++) {
      auto packed_val = pw_math_lp_in_on_chip_0_buf32_reconstruct_lp45_buf485457_write_channel.read();
