// AUTO GEN SODA TB
#include "dn3d_mini_opt_kernel.h"
#include <iostream>
#include <fstream>
...
int main() {
  ...
  ap_uint<BURST_WIDTH>* dn3d_mini = (ap_uint<BURST_WIDTH>*) malloc(sizeof(ap_uint<BURST_WIDTH>)*num_transfers);
  ...
  dn3d_mini_opt_kernel(dn3d_mini, f_oc, u_oc, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_dn3d_mini_opt_regression_result.csv", dn3d_mini, nrows, ncols, transfer_cols);
  free(f_oc);
  free(u_oc);
  free(dn3d_mini);
}
