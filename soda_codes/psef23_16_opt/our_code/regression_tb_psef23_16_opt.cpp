#include <fstream>
#include "psef23_16_opt.h"

int main() {
  ofstream in_pix("input_pixels_regression_result_psef23_16_opt.txt");
  ofstream fout("regression_result_psef23_16_opt.txt");
  HWStream<hw_uint<256> > in_update_0_read;
  HWStream<hw_uint<256> > psef23_16_update_0_write;


  // Loading input data
  // cmap    : { in_update_0[root = 0, in_0, in_1] -> in_off_chip[0, 0] : 0 <= in_0 <= 122 and 0 <= in_1 <= 1086 }
  // read map: { in_off_chip[0, 0] -> in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 122 and 0 <= in_1 <= 1086 }
  // rng     : { in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 122 and 0 <= in_1 <= 1086 }
  for (int i = 0; i < 133701; i++) {
    hw_uint<256> in_val;
  ...
  psef23_16_opt(in_update_0_read, psef23_16_update_0_write);

  for (int i = 0; i < 129600; i++) {
    hw_uint<256> actual = psef23_16_update_0_write.read();
