...
int main(int argc, char **argv) {
  ...
  const int jacdynl_2_update_0_write_pipe0_DATA_SIZE = num_epochs*2135250;
  const int jacdynl_2_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t jacdynl_2_update_0_write_size_bytes = jacdynl_2_update_0_write_BYTES_PER_PIXEL * jacdynl_2_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += jacdynl_2_update_0_write_size_bytes;
  total_size_bytes_written += jacdynl_2_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > jacdynl_2_update_0_write_pipe0(jacdynl_2_update_0_write_size_bytes);
  ...
  for (int i = 0; i < jacdynl_2_update_0_write_pipe0_DATA_SIZE; i++) {
#ifdef __FLOAT_OUTPUT__
    ((uint16_t*) (jacdynl_2_update_0_write_pipe0.data()))[i] = 0;
#else // __FLOAT_OUTPUT__
    ((uint16_t*) (jacdynl_2_update_0_write_pipe0.data()))[i] = 0;
#endif // __FLOAT_OUTPUT__
  }
  ...
  OCL_CHECK(err, cl::Buffer jacdynl_2_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, jacdynl_2_update_0_write_size_bytes, jacdynl_2_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, jacdynl_2_update_0_write_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({jacdynl_2_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("jacdynl_2_update_0_write_accel_result.csv");
  for (int i = 0; i < jacdynl_2_update_0_write_pipe0_DATA_SIZE; i++) {
#ifdef __FLOAT_OUTPUT__
    regression_result << bitcast<float, uint16_t>(((uint16_t*) (jacdynl_2_update_0_write_pipe0.data()))[i]) << std::endl;
#else // __FLOAT_OUTPUT__
    regression_result << ((uint16_t*) (jacdynl_2_update_0_write_pipe0.data()))[i] << std::endl;
#endif // __FLOAT_OUTPUT__
  }
