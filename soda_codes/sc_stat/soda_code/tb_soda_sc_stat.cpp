...
int main() {
  ...
  sc_stat_kernel(out, in_oc, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_sc_stat_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in_oc);
  free(out);
}
