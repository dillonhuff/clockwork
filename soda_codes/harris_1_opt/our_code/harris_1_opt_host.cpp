...
int main(int argc, char **argv) {
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "harris_1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer harris_1_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, harris_1_update_0_write_size_bytes, harris_1_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, harris_1_update_0_write_ocl_buf));

  int harris_1_update_0_write_size = harris_1_update_0_write_DATA_SIZE;
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, harris_1_update_0_write_size));
