...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > icfo_32_update_0_write_channel;
  // In lanes = 32
  ...
  icfo_32_opt(in_update_0_read_channel, icfo_32_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = icfo_32_update_0_write_channel.read();
  ...
  output.save_image("./images/icfo_32_opt_bmp_out.bmp");
}
