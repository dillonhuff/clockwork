...
int main(int argc, char **argv) {
  ...
  const int icfo_32_update_0_write_pipe0_DATA_SIZE = num_epochs*2441600;
  const int icfo_32_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t icfo_32_update_0_write_size_bytes = icfo_32_update_0_write_BYTES_PER_PIXEL * icfo_32_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += icfo_32_update_0_write_size_bytes;
  total_size_bytes_written += icfo_32_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > icfo_32_update_0_write_pipe0(icfo_32_update_0_write_size_bytes);
  ...
  for (int i = 0; i < icfo_32_update_0_write_pipe0_DATA_SIZE; i++) {
    ((uint16_t*) (icfo_32_update_0_write_pipe0.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "icfo_32_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer icfo_32_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, icfo_32_update_0_write_size_bytes, icfo_32_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, icfo_32_update_0_write_pipe0_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(2441600 / 32);
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({icfo_32_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("icfo_32_update_0_write_accel_result.csv");
  for (int i = 0; i < icfo_32_update_0_write_pipe0_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (icfo_32_update_0_write_pipe0.data()))[i] << std::endl;
  }
