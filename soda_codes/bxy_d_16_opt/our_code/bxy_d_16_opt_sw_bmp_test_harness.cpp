...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<256> > input_1_merged253_read_channel;
  HWStream<hw_uint<256> > bxy_d_16_1_merged259_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1082; r++) {
    for (int cl = 0; cl < 1104 / 16; cl++) {
      hw_uint<256> packed;
      {
      int c = 16*cl + 0;
      ...
      if (r < input.height() && c < input.width()) {
        ...
        input_1_merged253_read_channel.write(packed);
    }
  }
  bxy_d_16_opt(input_1_merged253_read_channel, bxy_d_16_1_merged259_write_channel);
  bitmap_image output(1088, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1088 / 16; cl++) {
      auto packed_val = bxy_d_16_1_merged259_write_channel.read();
  ...
  output.save_image("./images/bxy_d_16_opt_bmp_out.bmp");
}
