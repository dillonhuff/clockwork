...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > h10_16_300MHz_16_update_0_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1044; r++) {
    for (int cl = 0; cl < 1344 / 16; cl++) {
      hw_uint<512> packed;
      {
      int c = 16*cl + 0;
  ...
  h10_16_300MHz_16_opt(in_cc_update_0_read_channel, h10_16_300MHz_16_update_0_write_channel);
  bitmap_image output(1024, 1024);
  for (int r = 0; r < 1024; r++) {
    for (int cl = 0; cl < 1024 / 16; cl++) {
      auto packed_val = h10_16_300MHz_16_update_0_write_channel.read();
  ...
  output.save_image("./images/h10_16_300MHz_16_opt_bmp_out.bmp");
}
