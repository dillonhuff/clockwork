...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<32> > flt_stencil_1_update_0_write_channel;
  // In lanes = 1
  for (int r = 0; r < 34; r++) {
  ...
  flt_stencil_1_opt(in_cc_update_0_read_channel, flt_stencil_1_update_0_write_channel);
  bitmap_image output(32, 32);
  for (int r = 0; r < 32; r++) {
    for (int cl = 0; cl < 32 / 1; cl++) {
      auto packed_val = flt_stencil_1_update_0_write_channel.read();
  ...
  output.save_image("./images/flt_stencil_1_opt_bmp_out.bmp");
}
