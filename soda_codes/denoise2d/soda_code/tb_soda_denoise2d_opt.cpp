// AUTO GEN SODA TB
#include "denoise2d_opt_kernel.h"
#include <iostream>
#include <fstream>
...
int main() {
  ...
  denoise2d_opt_kernel(denoise2d, f_off_chip, u_off_chip, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_denoise2d_opt_regression_result.csv", denoise2d, nrows, ncols, transfer_cols);
  free(f_off_chip);
  free(u_off_chip);
  free(denoise2d);
}
