...
int main(int argc, char **argv) {
  ...
  const int denoise2d_update_0_write_pipe0_DATA_SIZE = num_epochs*900;
  const int denoise2d_update_0_write_pipe0_BYTES_PER_PIXEL = 32 / 8;
  size_t denoise2d_update_0_write_pipe0_size_bytes = denoise2d_update_0_write_pipe0_BYTES_PER_PIXEL * denoise2d_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += denoise2d_update_0_write_pipe0_size_bytes;
  ...
  const int u_update_0_read_pipe0_DATA_SIZE = num_epochs*1156;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > denoise2d_update_0_write_pipe0(denoise2d_update_0_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "denoise2d_naive_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer denoise2d_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, denoise2d_update_0_write_pipe0_size_bytes, denoise2d_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, denoise2d_update_0_write_pipe0_ocl_buf));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({denoise2d_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
{
    std::ofstream regression_result("denoise2d_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < denoise2d_update_0_write_pipe0_DATA_SIZE; i++) {
      regression_result << ((uint32_t*) (denoise2d_update_0_write_pipe0.data()))[i] << std::endl;
    }
}
