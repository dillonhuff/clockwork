...
int main(int argc, char **argv) {
  ...
  const int sbl_8_update_0_write_DATA_SIZE = num_epochs*2094752;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "sbl_8_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, sbl_8_update_0_write_ocl_buf));
