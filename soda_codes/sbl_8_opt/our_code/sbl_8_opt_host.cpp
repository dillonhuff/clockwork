...
int main(int argc, char **argv) {
  ...
  total_size_bytes += sbl_8_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > sbl_8_update_0_write(sbl_8_update_0_write_size_bytes);
  ...
  for (int i = 0; i < sbl_8_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (sbl_8_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "sbl_8_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer sbl_8_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, sbl_8_update_0_write_size_bytes, sbl_8_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, sbl_8_update_0_write_ocl_buf));

  int sbl_8_update_0_write_size = sbl_8_update_0_write_DATA_SIZE;
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, sbl_8_update_0_write_size));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({sbl_8_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));

  q.finish();

  std::ofstream regression_result("sbl_8_update_0_write_accel_result.csv");
  for (int i = 0; i < sbl_8_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (sbl_8_update_0_write.data()))[i] << std::endl;
  }
