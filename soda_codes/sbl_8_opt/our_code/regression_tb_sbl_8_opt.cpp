...
int main() {
  ofstream in_pix("input_pixels_regression_result_sbl_8_opt.txt");
  ofstream fout("regression_result_sbl_8_opt.txt");
  HWStream<hw_uint<128> > img_update_0_read;
  HWStream<hw_uint<128> > sbl_8_update_0_write;
  ...
  sbl_8_opt(img_update_0_read, sbl_8_update_0_write);

  for (int i = 0; i < 259200; i++) {
    hw_uint<128> actual = sbl_8_update_0_write.read();
