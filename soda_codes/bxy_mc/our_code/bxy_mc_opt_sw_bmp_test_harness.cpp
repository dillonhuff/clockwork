...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > bxy_mc_update_0_write_channel;
  // In lanes = 1
  ...
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 1; cl++) {
      hw_uint<16> packed;
      auto packed_val = bxy_mc_update_0_write_channel.read();
  ...
  output.save_image("./images/bxy_mc_opt_bmp_out.bmp");
}
