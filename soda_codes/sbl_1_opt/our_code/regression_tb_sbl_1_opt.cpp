...
int main() {
  ofstream in_pix("input_pixels_regression_result_sbl_1_opt.txt");
  ofstream fout("regression_result_sbl_1_opt.txt");
  HWStream<hw_uint<16> > img_update_0_read;
  HWStream<hw_uint<16> > sbl_1_update_0_write;
  ...
  sbl_1_opt(img_update_0_read, sbl_1_update_0_write);

  for (int i = 0; i < 2073600; i++) {
    hw_uint<16> actual = sbl_1_update_0_write.read();
