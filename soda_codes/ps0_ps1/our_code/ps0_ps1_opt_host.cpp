...
int main(int argc, char **argv) {
  ...
  total_size_bytes += ps1_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > ps0_update_0_write(ps0_update_0_write_size_bytes);
  std::vector<uint8_t, aligned_allocator<uint8_t> > ps1_update_0_write(ps1_update_0_write_size_bytes);
  ...
  for (int i = 0; i < ps0_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (ps0_update_0_write.data()))[i] = 0;
  }

  for (int i = 0; i < ps1_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (ps1_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "ps0_ps1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer ps0_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, ps0_update_0_write_size_bytes, ps0_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, ps0_update_0_write_ocl_buf));

  OCL_CHECK(err, cl::Buffer ps1_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, ps1_update_0_write_size_bytes, ps1_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(3, ps1_update_0_write_ocl_buf));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({ps0_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({ps1_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
{
    std::ofstream regression_result("ps0_update_0_write_accel_result.csv");
    for (int i = 0; i < ps0_update_0_write_DATA_SIZE; i++) {
      regression_result << ((uint16_t*) (ps0_update_0_write.data()))[i] << std::endl;
    }
}
{
    std::ofstream regression_result("ps1_update_0_write_accel_result.csv");
    for (int i = 0; i < ps1_update_0_write_DATA_SIZE; i++) {
      regression_result << ((uint16_t*) (ps1_update_0_write.data()))[i] << std::endl;
    }
}
