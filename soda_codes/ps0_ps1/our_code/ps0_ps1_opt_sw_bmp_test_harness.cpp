...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<32> > in_off_chip0_update_0_read_channel;
  HWStream<hw_uint<32> > in_off_chip1_update_0_read_channel;
  HWStream<hw_uint<32> > ps0_update_0_write_channel;
  HWStream<hw_uint<32> > ps1_update_0_write_channel;
  // In lanes = 2
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 960 / 2; cl++) {
      hw_uint<32> packed;
      {
      int c = 2*cl + 0;
