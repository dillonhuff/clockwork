...
int main() {
  ofstream in_pix("input_pixels_regression_result_bxy_ur_2_opt.txt");
  ofstream fout("regression_result_bxy_ur_2_opt.txt");
  HWStream<hw_uint<32> > input_update_0_read;
  HWStream<hw_uint<32> > bxy_ur_2_update_0_write;
  ...
  bxy_ur_2_opt(input_update_0_read, bxy_ur_2_update_0_write);

  for (int i = 0; i < 1036800; i++) {
    hw_uint<32> actual = bxy_ur_2_update_0_write.read();
