...
#include <fstream>
#include <vector>
#include "bxy_noinit_ln_32_opt.h"
...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > bxy_noinit_ln_32_update_0_write_channel;
  // In lanes = 32
  ...
  bxy_noinit_ln_32_opt(input_update_0_read_channel, bxy_noinit_ln_32_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = bxy_noinit_ln_32_update_0_write_channel.read();
  ...
  output.save_image("./images/bxy_noinit_ln_32_opt_bmp_out.bmp");
}
