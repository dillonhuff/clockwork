...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<32> > gp_fpga_16_update_0_write_channel;
  // In lanes = 16
  ...
  gp_fpga_16_opt(in_update_0_read_channel, gp_fpga_16_update_0_write_channel);
  bitmap_image output(240, 135);
  for (int r = 0; r < 135; r++) {
    for (int cl = 0; cl < 240 / 2; cl++) {
      auto packed_val = gp_fpga_16_update_0_write_channel.read();
  ...
  output.save_image("./images/gp_fpga_16_opt_bmp_out.bmp");
}
