...
#include <fstream>
#include <vector>
#include "cp_32_opt_d.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<512> > raw_1_merged2235_read_channel;
  HWStream<hw_uint<512> > cp_32_1_merged2265_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 1184 / 32; cl++) {
      hw_uint<512> packed;
      {
      int c = 32*cl + 0;
      ...
      if (r < input.height() && c < input.width()) {
        ...
        raw_1_merged2235_read_channel.write(packed);
    }
  }
  cp_32_opt_d(raw_1_merged2235_read_channel, cp_32_1_merged2265_write_channel);
  bitmap_image output(1088, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1088 / 32; cl++) {
      auto packed_val = cp_32_1_merged2265_write_channel.read();
  ...
  output.save_image("./images/cp_32_opt_d_bmp_out.bmp");
}
