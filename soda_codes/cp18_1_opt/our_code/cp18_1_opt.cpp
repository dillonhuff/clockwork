...
#include "cp18_1_opt_compute_units.h"
...
inline hw_uint<16> cp18_1_rd0_select(demosaic_diff_cache& demosaic_diff, int d0, int d1) {
#ifdef __VIVADO_SYNTH__
#endif //__VIVADO_SYNTH__
  // cp18_1_rd0 read pattern: { cp18_1_update_0[d0, d1] -> demosaic_diff[d0, d1] : 0 <= d0 <= 1919 and 0 <= d1 <= 1079 }
  // Read schedule : { cp18_1_update_0[d0, d1] -> [5 + d1, 5 + d0, 7] : 0 <= d0 <= 1919 and 0 <= d1 <= 1079 }
...
// # of bundles = 2
// cp18_1_update_0_read
//	cp18_1_rd0
inline hw_uint<16> demosaic_diff_cp18_1_update_0_read_bundle_read(demosaic_diff_cache& demosaic_diff, int d0, int d1) {
  // # of ports in bundle: 1
    // cp18_1_rd0

	hw_uint<16> result;
	hw_uint<16> cp18_1_rd0_res = cp18_1_rd0_select(demosaic_diff, d0, d1);
	set_at<0, 16>(result, cp18_1_rd0_res);
	return result;
}
...
inline void cp18_1_update_0(demosaic_diff_cache& demosaic_diff, HWStream<hw_uint<16> >& /* buffer_args num ports = 1 */cp18_1, int d0, int d1) {
	// Consume: demosaic_diff
	auto demosaic_diff_0_c__0_value = demosaic_diff_cp18_1_update_0_read_bundle_read(demosaic_diff/* source_delay */, d0, d1);

#ifndef __VIVADO_SYNTH__
#endif //__VIVADO_SYNTH__

	auto compute_result = cp18_1_generated_compute_unrolled_1(demosaic_diff_0_c__0_value);
	// Produce: cp18_1
	cp18_1.write(compute_result);
...
// Driver function
void cp18_1_opt(HWStream<hw_uint<16> >& /* get_args num ports = 1 */raw_oc, HWStream<hw_uint<16> >& /* get_args num ports = 1 */cp18_1, int num_epochs) {

#ifndef __VIVADO_SYNTH__
  ofstream debug_file("cp18_1_opt_debug.csv");
...
#pragma HLS inline recursive
  ...
  for (int epoch = 0; epoch < 30; epoch++) {
	  // Schedules...
	    // cp18_1_update_0 -> [1*d1*1*1 + 1*5,1*d0*1*1 + 1*5,1*7]
	...
	demosaic_diff_update_0(demosaic, demosaic_by, demosaic_diff, (c1 - 5) / 1, (c0 - 5) / 1);
	    ...
	    if ((5 <= c1 && c1 <= 1924) && ((c1 - 5) % 1 == 0) && (5 <= c0 && c0 <= 1084) && ((c0 - 5) % 1 == 0)) {
	cp18_1_update_0(demosaic_diff, cp18_1, (c1 - 5) / 1, (c0 - 5) / 1);
	    }
...
void cp18_1_opt(HWStream<hw_uint<16> >& /* get_args num ports = 1 */raw_oc, HWStream<hw_uint<16> >& /* get_args num ports = 1 */cp18_1) {

  cp18_1_opt(raw_oc, cp18_1, 1);
}
#ifdef __VIVADO_SYNTH__
#include "cp18_1_opt.h"

const int cp18_1_update_0_write_num_transfers = 2073600;
...
static void write_cp18_1_update_0_write(hw_uint<16>* output, HWStream<hw_uint<16> >& v, const int size) {
  hw_uint<16> burst_reg;
  int num_transfers = cp18_1_update_0_write_num_transfers*30;
  for (int i = 0; i < num_transfers; i++) {
    #pragma HLS pipeline II=1
    burst_reg = v.read();
    output[i] = burst_reg;
...
void cp18_1_opt_accel(hw_uint<16>* raw_update_0_read, hw_uint<16>* cp18_1_update_0_write, const int size) { 
#pragma HLS dataflow
#pragma HLS INTERFACE m_axi port = raw_update_0_read offset = slave depth = 65536 bundle = gmem0
#pragma HLS INTERFACE m_axi port = cp18_1_update_0_write offset = slave depth = 65536 bundle = gmem1

#pragma HLS INTERFACE s_axilite port = raw_update_0_read bundle = control
#pragma HLS INTERFACE s_axilite port = cp18_1_update_0_write bundle = control
#pragma HLS INTERFACE s_axilite port = size bundle = control
#pragma HLS INTERFACE s_axilite port = return bundle = control

  static HWStream<hw_uint<16> > raw_update_0_read_channel;
  static HWStream<hw_uint<16> > cp18_1_update_0_write_channel;

  read_raw_update_0_read(raw_update_0_read, raw_update_0_read_channel, size);

  cp18_1_opt(raw_update_0_read_channel, cp18_1_update_0_write_channel, size);

  write_cp18_1_update_0_write(cp18_1_update_0_write, cp18_1_update_0_write_channel, size);
}
