...
int main(int argc, char **argv) {
  ...
  const int bxy_ur_8_update_0_write_DATA_SIZE = num_epochs*2086096;
  const int bxy_ur_8_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t bxy_ur_8_update_0_write_size_bytes = bxy_ur_8_update_0_write_BYTES_PER_PIXEL * bxy_ur_8_update_0_write_DATA_SIZE;

  total_size_bytes += bxy_ur_8_update_0_write_size_bytes;
  const int input_update_0_read_DATA_SIZE = num_epochs*2086096;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "bxy_ur_8_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer bxy_ur_8_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, bxy_ur_8_update_0_write_size_bytes, bxy_ur_8_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, bxy_ur_8_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(2086096 / 8);
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({bxy_ur_8_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
