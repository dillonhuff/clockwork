...
int main() {
  ofstream in_pix("input_pixels_regression_result_bxy_ur_8_opt.txt");
  ofstream fout("regression_result_bxy_ur_8_opt.txt");
  HWStream<hw_uint<128> > input_update_0_read;
  HWStream<hw_uint<128> > bxy_ur_8_update_0_write;
  ...
  bxy_ur_8_opt(input_update_0_read, bxy_ur_8_update_0_write);

  for (int i = 0; i < 259200; i++) {
    hw_uint<128> actual = bxy_ur_8_update_0_write.read();
