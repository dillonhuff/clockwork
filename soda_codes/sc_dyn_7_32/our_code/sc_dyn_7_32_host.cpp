...
int main(int argc, char **argv) {
  ...
  const int pw_math_in47_write_pipe0_BYTES_PER_PIXEL = 32 / 8;
  size_t pw_math_in47_write_pipe0_size_bytes = pw_math_in47_write_pipe0_BYTES_PER_PIXEL * pw_math_in47_write_pipe0_DATA_SIZE;

  total_size_bytes += pw_math_in47_write_pipe0_size_bytes;
  ...
  OCL_CHECK(err, cl::Buffer pw_math_in47_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, pw_math_in47_write_pipe0_size_bytes, pw_math_in47_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, pw_math_in47_write_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({pw_math_in47_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
{
    std::ofstream regression_result("pw_math_in47_write_pipe0_accel_result.csv");
    for (int i = 0; i < pw_math_in47_write_pipe0_DATA_SIZE; i++) {
