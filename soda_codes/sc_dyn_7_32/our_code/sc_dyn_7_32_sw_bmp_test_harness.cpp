...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<32> > pw_math_in47_write_channel;
  // In lanes = 1
  for (int r = 0; r < 128; r++) {
  ...
  sc_dyn_7_32(pw_math_in_oc03_read_channel, pw_math_in47_write_channel);
  bitmap_image output(128, 128);
  for (int r = 0; r < 128; r++) {
    for (int cl = 0; cl < 128 / 1; cl++) {
      auto packed_val = pw_math_in47_write_channel.read();
