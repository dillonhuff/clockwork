...
int main() {
  srand(234);
  const int nrows = 1094;
  const int ncols = 2032;
  ...
  sc_dyn_7_32_kernel(out, in_oc, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_sc_dyn_7_32_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in_oc);
  free(out);
}
