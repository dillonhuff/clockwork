...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<256> > in_1_merged1747_read_channel;
  HWStream<hw_uint<256> > jac_d_16_1_merged1797_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1110; r++) {
    for (int cl = 0; cl < 1328 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        in_1_merged1747_read_channel.write(packed);
    }
  }
  jac_d_16_opt(in_1_merged1747_read_channel, jac_d_16_1_merged1797_write_channel);
  bitmap_image output(1088, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1088 / 16; cl++) {
      auto packed_val = jac_d_16_1_merged1797_write_channel.read();
  ...
  output.save_image("./images/jac_d_16_opt_bmp_out.bmp");
}
