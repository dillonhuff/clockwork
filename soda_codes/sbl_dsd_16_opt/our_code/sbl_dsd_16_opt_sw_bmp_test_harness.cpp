...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<256> > img_1_merged430_read_channel;
  HWStream<hw_uint<256> > sbl_dsd_16_1_merged439_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1082; r++) {
    for (int cl = 0; cl < 1104 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        img_1_merged430_read_channel.write(packed);
    }
  }
  sbl_dsd_16_opt(img_1_merged430_read_channel, sbl_dsd_16_1_merged439_write_channel);
  bitmap_image output(1088, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1088 / 16; cl++) {
      auto packed_val = sbl_dsd_16_1_merged439_write_channel.read();
  ...
  output.save_image("./images/sbl_dsd_16_opt_bmp_out.bmp");
}
