// AUTO GEN SODA TB
#include "two_in_blnd_nb_kernel.h"
#include <iostream>
#include <fstream>
...
int main() {
  ...
  two_in_blnd_nb_kernel(out, in0_oc, in1_oc, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_two_in_blnd_nb_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in0_oc);
  free(in1_oc);
  free(out);
}
