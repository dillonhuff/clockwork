...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<128> > ef_8_update_0_write_channel;
  // In lanes = 8
  for (int r = 0; r < 1927; r++) {
    for (int cl = 0; cl < 1128 / 8; cl++) {
      hw_uint<128> packed;
      {
      int c = 8*cl + 0;
  ...
  ef_8_opt(in_update_0_read_channel, ef_8_update_0_write_channel);
  bitmap_image output(1080, 1920);
  for (int r = 0; r < 1920; r++) {
    for (int cl = 0; cl < 1080 / 8; cl++) {
      auto packed_val = ef_8_update_0_write_channel.read();
  ...
  output.save_image("./images/ef_8_opt_bmp_out.bmp");
}
