...
int main() {
  ofstream in_pix("input_pixels_regression_result_psef_gpct3_4_opt.txt");
  ofstream fout("regression_result_psef_gpct3_4_opt.txt");
  HWStream<hw_uint<64> > in_update_0_read;
  HWStream<hw_uint<64> > psef_gpct3_4_update_0_write;
  ...
  psef_gpct3_4_opt(in_update_0_read, psef_gpct3_4_update_0_write);

  for (int i = 0; i < 518400; i++) {
    hw_uint<64> actual = psef_gpct3_4_update_0_write.read();
