...
int main(int argc, char **argv) {
  ...
  const int idstream_1_update_0_write_DATA_SIZE = num_epochs*2073600;
  const int idstream_1_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t idstream_1_update_0_write_size_bytes = idstream_1_update_0_write_BYTES_PER_PIXEL * idstream_1_update_0_write_DATA_SIZE;

  total_size_bytes += idstream_1_update_0_write_size_bytes;
  total_size_bytes_written += idstream_1_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > idstream_1_update_0_write(idstream_1_update_0_write_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "idstream_1_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer idstream_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, idstream_1_update_0_write_size_bytes, idstream_1_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, idstream_1_update_0_write_pipe0_ocl_buf));
...
nsduration = end - start;
  ...
  std::ofstream regression_result("idstream_1_update_0_write_accel_result.csv");
  for (int i = 0; i < idstream_1_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (idstream_1_update_0_write.data()))[i] << std::endl;
  }
