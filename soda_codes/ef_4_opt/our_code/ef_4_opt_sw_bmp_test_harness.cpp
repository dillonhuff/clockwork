...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<64> > ef_4_update_0_write_channel;
  // In lanes = 4
  for (int r = 0; r < 1927; r++) {
    for (int cl = 0; cl < 1100 / 4; cl++) {
      hw_uint<64> packed;
      {
      int c = 4*cl + 0;
  ...
  ef_4_opt(in_update_0_read_channel, ef_4_update_0_write_channel);
  bitmap_image output(1080, 1920);
  for (int r = 0; r < 1920; r++) {
    for (int cl = 0; cl < 1080 / 4; cl++) {
      auto packed_val = ef_4_update_0_write_channel.read();
  ...
  output.save_image("./images/ef_4_opt_bmp_out.bmp");
}
