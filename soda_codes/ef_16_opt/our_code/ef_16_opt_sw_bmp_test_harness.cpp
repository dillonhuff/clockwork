...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<256> > ef_16_update_0_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1927; r++) {
    for (int cl = 0; cl < 1184 / 16; cl++) {
      hw_uint<256> packed;
      {
      int c = 16*cl + 0;
  ...
  ef_16_opt(in_update_0_read_channel, ef_16_update_0_write_channel);
  bitmap_image output(1088, 1920);
  for (int r = 0; r < 1920; r++) {
    for (int cl = 0; cl < 1088 / 16; cl++) {
      auto packed_val = ef_16_update_0_write_channel.read();
  ...
  output.save_image("./images/ef_16_opt_bmp_out.bmp");
}
