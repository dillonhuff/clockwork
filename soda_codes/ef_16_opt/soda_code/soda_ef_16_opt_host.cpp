...
int main(int argc, char **argv) {
  ...
  const int ef_16_update_0_write_pipe0_DATA_SIZE = num_epochs*2281568;
  const int ef_16_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t ef_16_update_0_write_size_bytes = ef_16_update_0_write_BYTES_PER_PIXEL * ef_16_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += ef_16_update_0_write_size_bytes;
  total_size_bytes_written += ef_16_update_0_write_size_bytes;
  const int in_update_0_read_pipe0_DATA_SIZE = num_epochs*2281568;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > ef_16_update_0_write_pipe0(ef_16_update_0_write_size_bytes);
  ...
  for (int i = 0; i < ef_16_update_0_write_pipe0_DATA_SIZE; i++) {
#ifdef __FLOAT_OUTPUT__
    ((uint16_t*) (ef_16_update_0_write_pipe0.data()))[i] = 0;
#else // __FLOAT_OUTPUT__
    ((uint16_t*) (ef_16_update_0_write_pipe0.data()))[i] = 0;
#endif // __FLOAT_OUTPUT__
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "ef_16_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer ef_16_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, ef_16_update_0_write_size_bytes, ef_16_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, ef_16_update_0_write_pipe0_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(2281568 / 16);
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({ef_16_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("ef_16_update_0_write_accel_result.csv");
  for (int i = 0; i < ef_16_update_0_write_pipe0_DATA_SIZE; i++) {
#ifdef __FLOAT_OUTPUT__
    regression_result << bitcast<float, uint16_t>(((uint16_t*) (ef_16_update_0_write_pipe0.data()))[i]) << std::endl;
#else // __FLOAT_OUTPUT__
    regression_result << ((uint16_t*) (ef_16_update_0_write_pipe0.data()))[i] << std::endl;
#endif // __FLOAT_OUTPUT__
  }
