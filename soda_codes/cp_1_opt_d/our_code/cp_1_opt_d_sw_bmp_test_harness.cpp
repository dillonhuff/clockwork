...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > raw_update_0_sm99_0143_read_channel;
  HWStream<hw_uint<16> > cp_1_update_0_sm109_0163_write_channel;
  // In lanes = 1
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 1088 / 1; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        raw_update_0_sm99_0143_read_channel.write(packed);
    }
  }
  cp_1_opt_d(raw_update_0_sm99_0143_read_channel, cp_1_update_0_sm109_0163_write_channel);
  bitmap_image output(1080, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1080 / 1; cl++) {
      auto packed_val = cp_1_update_0_sm109_0163_write_channel.read();
