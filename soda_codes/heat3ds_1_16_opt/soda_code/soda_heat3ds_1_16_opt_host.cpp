...
int main(int argc, char **argv) {
  ...
  const int heat3ds_1_16_update_0_write_pipe0_DATA_SIZE = num_epochs*73984;
  const int heat3ds_1_16_update_0_write_BYTES_PER_PIXEL = 32 / 8;
  size_t heat3ds_1_16_update_0_write_size_bytes = heat3ds_1_16_update_0_write_BYTES_PER_PIXEL * heat3ds_1_16_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += heat3ds_1_16_update_0_write_size_bytes;
  total_size_bytes_written += heat3ds_1_16_update_0_write_size_bytes;
  const int in_cc_update_0_read_pipe0_DATA_SIZE = num_epochs*73984;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > heat3ds_1_16_update_0_write_pipe0(heat3ds_1_16_update_0_write_size_bytes);
  ...
  for (int i = 0; i < heat3ds_1_16_update_0_write_pipe0_DATA_SIZE; i++) {
#ifdef __FLOAT_OUTPUT__
    ((uint32_t*) (heat3ds_1_16_update_0_write_pipe0.data()))[i] = 0;
#else // __FLOAT_OUTPUT__
    ((uint32_t*) (heat3ds_1_16_update_0_write_pipe0.data()))[i] = 0;
#endif // __FLOAT_OUTPUT__
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "heat3ds_1_16_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer heat3ds_1_16_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, heat3ds_1_16_update_0_write_size_bytes, heat3ds_1_16_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, heat3ds_1_16_update_0_write_pipe0_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(73984 / 16);
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({heat3ds_1_16_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("heat3ds_1_16_update_0_write_accel_result.csv");
  for (int i = 0; i < heat3ds_1_16_update_0_write_pipe0_DATA_SIZE; i++) {
#ifdef __FLOAT_OUTPUT__
    regression_result << bitcast<float, uint32_t>(((uint32_t*) (heat3ds_1_16_update_0_write_pipe0.data()))[i]) << std::endl;
#else // __FLOAT_OUTPUT__
    regression_result << ((uint32_t*) (heat3ds_1_16_update_0_write_pipe0.data()))[i] << std::endl;
#endif // __FLOAT_OUTPUT__
  }
