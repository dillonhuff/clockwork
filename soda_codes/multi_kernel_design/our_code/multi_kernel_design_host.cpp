...
int main(int argc, char **argv) {
  ...
  const int pw_math_gray47_write_pipe0_BYTES_PER_PIXEL = 32 / 8;
  size_t pw_math_gray47_write_pipe0_size_bytes = pw_math_gray47_write_pipe0_BYTES_PER_PIXEL * pw_math_gray47_write_pipe0_DATA_SIZE;

  total_size_bytes += pw_math_gray47_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > pw_math_gray47_write_pipe0(pw_math_gray47_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "multi_kernel_design_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer pw_math_gray47_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, pw_math_gray47_write_pipe0_size_bytes, pw_math_gray47_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, pw_math_gray47_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("pw_math_gray47_write_pipe0_accel_result.csv");
    for (int i = 0; i < pw_math_gray47_write_pipe0_DATA_SIZE; i++) {
