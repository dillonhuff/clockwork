...
int main(int argc, char **argv) {
  ...
  const int pw_math_gray47_write_DATA_SIZE = num_epochs*16;
  const int pw_math_gray47_write_BYTES_PER_PIXEL = 32 / 8;
  size_t pw_math_gray47_write_size_bytes = pw_math_gray47_write_BYTES_PER_PIXEL * pw_math_gray47_write_DATA_SIZE;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > pw_math_gray47_write(pw_math_gray47_write_size_bytes);
  ...
  for (int i = 0; i < pw_math_gray47_write_DATA_SIZE; i++) {
    ((uint32_t*) (pw_math_gray47_write.data()))[i] = 0;
  }
...
nsduration = end - start;
  ...
  for (int i = 0; i < pw_math_gray47_write_DATA_SIZE; i++) {
    regression_result << ((uint32_t*) (pw_math_gray47_write.data()))[i] << std::endl;
  }
