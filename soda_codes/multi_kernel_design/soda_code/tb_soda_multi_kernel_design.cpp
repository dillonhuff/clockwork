// AUTO GEN SODA TB
#include "multi_kernel_design_kernel.h"
#include <iostream>
#include <fstream>
...
int main() {
  ...
  multi_kernel_design_kernel(out, in, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_multi_kernel_design_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in);
  free(out);
}
