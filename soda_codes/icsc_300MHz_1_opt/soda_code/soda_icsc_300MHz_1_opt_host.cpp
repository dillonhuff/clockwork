...
int main(int argc, char **argv) {
  ...
  const int icsc_300MHz_1_update_0_write_pipe0_DATA_SIZE = num_epochs*2114600;
  const int icsc_300MHz_1_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t icsc_300MHz_1_update_0_write_size_bytes = icsc_300MHz_1_update_0_write_BYTES_PER_PIXEL * icsc_300MHz_1_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += icsc_300MHz_1_update_0_write_size_bytes;
  total_size_bytes_written += icsc_300MHz_1_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > icsc_300MHz_1_update_0_write_pipe0(icsc_300MHz_1_update_0_write_size_bytes);
  ...
  for (int i = 0; i < icsc_300MHz_1_update_0_write_pipe0_DATA_SIZE; i++) {
    ((uint16_t*) (icsc_300MHz_1_update_0_write_pipe0.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "icsc_300MHz_1_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer icsc_300MHz_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, icsc_300MHz_1_update_0_write_size_bytes, icsc_300MHz_1_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, icsc_300MHz_1_update_0_write_pipe0_ocl_buf));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({icsc_300MHz_1_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("icsc_300MHz_1_update_0_write_accel_result.csv");
  for (int i = 0; i < icsc_300MHz_1_update_0_write_pipe0_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (icsc_300MHz_1_update_0_write_pipe0.data()))[i] << std::endl;
  }
