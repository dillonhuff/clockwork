...
#include <fstream>
#include <vector>
#include "jac4_32_opt_dis.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<512> > in_1_merged3379_read_channel;
  HWStream<hw_uint<512> > jac4_32_1_merged3427_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1095; r++) {
    for (int cl = 0; cl < 2400 / 32; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        in_1_merged3379_read_channel.write(packed);
    }
  }
  jac4_32_opt_dis(in_1_merged3379_read_channel, jac4_32_1_merged3427_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = jac4_32_1_merged3427_write_channel.read();
  ...
  output.save_image("./images/jac4_32_opt_dis_bmp_out.bmp");
}
