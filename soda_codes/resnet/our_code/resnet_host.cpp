...
int main(int argc, char **argv) {
  ...
  const int op_hcompute_hw_input_global_wrapper_stencil_read_pipe0_DATA_SIZE = num_epochs*7200;
  ...
  const int op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_DATA_SIZE = num_epochs*216;
  const int op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_size_bytes = op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_BYTES_PER_PIXEL * op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_DATA_SIZE;

  total_size_bytes += op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_size_bytes;
  const int op_hcompute_hw_output_stencil_write_pipe0_DATA_SIZE = num_epochs*2352;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0(op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "resnet_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_size_bytes, op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, op_hcompute_hw_output_stencil_write_pipe0_ocl_buf));


  OCL_CHECK(err, err = krnl_vector_add.setArg(3, num_epochs));

  std::cout << "Migrating memory" << std::endl;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({op_hcompute_hw_input_global_wrapper_stencil_read_pipe0_ocl_buf, op_hcompute_hw_kernel_global_wrapper_stencil_read_pipe0_ocl_buf}, 0));
