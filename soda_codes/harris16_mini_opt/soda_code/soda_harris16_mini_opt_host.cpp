...
int main(int argc, char **argv) {
  ...
  const int harris16_mini_update_0_write_DATA_SIZE = num_epochs*1296;
  const int harris16_mini_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  ...
  const int img_update_0_read_DATA_SIZE = num_epochs*1296;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > harris16_mini_update_0_write(harris16_mini_update_0_write_size_bytes);
  ...
  for (int i = 0; i < harris16_mini_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (harris16_mini_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "harris16_mini_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer harris16_mini_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, harris16_mini_update_0_write_size_bytes, harris16_mini_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, harris16_mini_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(1296 / 1);
...
nsduration = end - start;
  ...
  std::ofstream regression_result("harris16_mini_update_0_write_accel_result.csv");
  for (int i = 0; i < harris16_mini_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (harris16_mini_update_0_write.data()))[i] << std::endl;
  }
