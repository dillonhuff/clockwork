...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > in_update_0_sm138_0197_read_channel;
  HWStream<hw_uint<16> > jacdynl_1_update_0_sm154_0229_write_channel;
  // In lanes = 1
  for (int r = 0; r < 1950; r++) {
    for (int cl = 0; cl < 1095 / 1; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        in_update_0_sm138_0197_read_channel.write(packed);
    }
  }
  jacdynl_1_opt(in_update_0_sm138_0197_read_channel, jacdynl_1_update_0_sm154_0229_write_channel);
  bitmap_image output(1080, 1920);
  for (int r = 0; r < 1920; r++) {
    for (int cl = 0; cl < 1080 / 1; cl++) {
      auto packed_val = jacdynl_1_update_0_sm154_0229_write_channel.read();
