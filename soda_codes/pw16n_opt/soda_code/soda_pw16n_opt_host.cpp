...
int main(int argc, char **argv) {
  ...
  const int input_update_0_read_DATA_SIZE = num_epochs*2073600;
  ...
  const int pw16n_update_0_write_DATA_SIZE = num_epochs*2073600;
  const int pw16n_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t pw16n_update_0_write_size_bytes = pw16n_update_0_write_BYTES_PER_PIXEL * pw16n_update_0_write_DATA_SIZE;

  total_size_bytes += pw16n_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > pw16n_update_0_write(pw16n_update_0_write_size_bytes);
  ...
  for (int i = 0; i < pw16n_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (pw16n_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "pw16n_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer pw16n_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, pw16n_update_0_write_size_bytes, pw16n_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, pw16n_update_0_write_ocl_buf));
...
nsduration = end - start;
  ...
  std::ofstream regression_result("pw16n_update_0_write_accel_result.csv");
  for (int i = 0; i < pw16n_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (pw16n_update_0_write.data()))[i] << std::endl;
  }
