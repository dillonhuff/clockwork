...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<64> > t1_1_merged12_read_channel;
  HWStream<hw_uint<64> > pw5_2_1_merged15_write_channel;
  // In lanes = 2
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 2; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        t1_1_merged12_read_channel.write(packed);
    }
  }
  pw5_2_opt(t1_1_merged12_read_channel, pw5_2_1_merged15_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 2; cl++) {
      auto packed_val = pw5_2_1_merged15_write_channel.read();
  ...
  output.save_image("./images/pw5_2_opt_bmp_out.bmp");
}
