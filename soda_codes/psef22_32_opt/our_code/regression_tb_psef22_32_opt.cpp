#include <fstream>
#include "psef22_32_opt.h"

int main() {
  ofstream in_pix("input_pixels_regression_result_psef22_32_opt.txt");
  ofstream fout("regression_result_psef22_32_opt.txt");
  HWStream<hw_uint<512> > in_update_0_read;
  HWStream<hw_uint<512> > psef22_32_update_0_write;


  // Loading input data
  // cmap    : { in_update_0[root = 0, in_0, in_1] -> in_off_chip[0, 0] : 0 <= in_0 <= 42 and 0 <= in_1 <= 1262 }
  // read map: { in_off_chip[0, 0] -> in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 42 and 0 <= in_1 <= 1262 }
  // rng     : { in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 42 and 0 <= in_1 <= 1262 }
  for (int i = 0; i < 54309; i++) {
    hw_uint<512> in_val;
  ...
  psef22_32_opt(in_update_0_read, psef22_32_update_0_write);

  for (int i = 0; i < 50000; i++) {
    hw_uint<512> actual = psef22_32_update_0_write.read();
