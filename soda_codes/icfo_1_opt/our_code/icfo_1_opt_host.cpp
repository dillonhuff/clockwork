...
int main(int argc, char **argv) {
  ...
  const int icfo_1_update_0_write_pipe0_DATA_SIZE = num_epochs*2073600;
  const int icfo_1_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t icfo_1_update_0_write_pipe0_size_bytes = icfo_1_update_0_write_pipe0_BYTES_PER_PIXEL * icfo_1_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += icfo_1_update_0_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > icfo_1_update_0_write_pipe0(icfo_1_update_0_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "icfo_1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer icfo_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, icfo_1_update_0_write_pipe0_size_bytes, icfo_1_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, icfo_1_update_0_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("icfo_1_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < icfo_1_update_0_write_pipe0_DATA_SIZE; i++) {
