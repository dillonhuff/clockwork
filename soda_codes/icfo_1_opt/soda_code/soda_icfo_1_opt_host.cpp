...
int main(int argc, char **argv) {
  ...
  const int icfo_1_update_0_write_pipe0_DATA_SIZE = num_epochs*2103700;
  const int icfo_1_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t icfo_1_update_0_write_size_bytes = icfo_1_update_0_write_BYTES_PER_PIXEL * icfo_1_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += icfo_1_update_0_write_size_bytes;
  total_size_bytes_written += icfo_1_update_0_write_size_bytes;
  const int in_update_0_read_pipe0_DATA_SIZE = num_epochs*2103700;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > icfo_1_update_0_write_pipe0(icfo_1_update_0_write_size_bytes);
  ...
  for (int i = 0; i < icfo_1_update_0_write_pipe0_DATA_SIZE; i++) {
    ((uint16_t*) (icfo_1_update_0_write_pipe0.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "icfo_1_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer icfo_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, icfo_1_update_0_write_size_bytes, icfo_1_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, icfo_1_update_0_write_pipe0_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(2103700 / 1);
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({icfo_1_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("icfo_1_update_0_write_accel_result.csv");
  for (int i = 0; i < icfo_1_update_0_write_pipe0_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (icfo_1_update_0_write_pipe0.data()))[i] << std::endl;
  }
