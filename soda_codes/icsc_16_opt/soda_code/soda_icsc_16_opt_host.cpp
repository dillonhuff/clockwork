...
int main(int argc, char **argv) {
  ...
  const int icsc_16_update_0_write_DATA_SIZE = num_epochs*2441600;
  const int icsc_16_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t icsc_16_update_0_write_size_bytes = icsc_16_update_0_write_BYTES_PER_PIXEL * icsc_16_update_0_write_DATA_SIZE;

  total_size_bytes += icsc_16_update_0_write_size_bytes;
  total_size_bytes_written += icsc_16_update_0_write_size_bytes;
  const int in_update_0_read_DATA_SIZE = num_epochs*2441600;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > icsc_16_update_0_write(icsc_16_update_0_write_size_bytes);
  ...
  for (int i = 0; i < icsc_16_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (icsc_16_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "icsc_16_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer icsc_16_update_0_writepipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, icsc_16_update_0_write_size_bytes, icsc_16_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, icsc_16_update_0_writepipe0_ocl_buf));
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({icsc_16_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
  ...
  std::ofstream regression_result("icsc_16_update_0_write_accel_result.csv");
  for (int i = 0; i < icsc_16_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (icsc_16_update_0_write.data()))[i] << std::endl;
  }
