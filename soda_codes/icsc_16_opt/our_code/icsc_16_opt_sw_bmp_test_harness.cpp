...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<256> > icsc_16_update_0_write_channel;
  // In lanes = 16
  ...
  icsc_16_opt(in_update_0_read_channel, icsc_16_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 16; cl++) {
      auto packed_val = icsc_16_update_0_write_channel.read();
  ...
  output.save_image("./images/icsc_16_opt_bmp_out.bmp");
}
