...
int main() {
  ofstream in_pix("input_pixels_regression_result_sbl_ur_16_opt.txt");
  ofstream fout("regression_result_sbl_ur_16_opt.txt");
  HWStream<hw_uint<256> > img_update_0_read;
  HWStream<hw_uint<256> > sbl_ur_16_update_0_write;
  ...
  sbl_ur_16_opt(img_update_0_read, sbl_ur_16_update_0_write);

  for (int i = 0; i < 129600; i++) {
    hw_uint<256> actual = sbl_ur_16_update_0_write.read();
