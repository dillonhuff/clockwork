...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > icsc_8s_1_update_0_write_channel;
  // In lanes = 1
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 1936 / 1; cl++) {
      hw_uint<16> packed;
      {
      int c = 1*cl + 0;
  ...
  icsc_8s_1_opt(in_update_0_read_channel, icsc_8s_1_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 1; cl++) {
      auto packed_val = icsc_8s_1_update_0_write_channel.read();
  ...
  output.save_image("./images/icsc_8s_1_opt_bmp_out.bmp");
}
