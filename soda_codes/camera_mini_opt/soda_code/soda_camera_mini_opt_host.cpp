...
int main(int argc, char **argv) {
  ...
  const int camera_mini_update_0_write_DATA_SIZE = num_epochs*4104;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "camera_mini_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, camera_mini_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(4104 / 1);
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({camera_mini_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
