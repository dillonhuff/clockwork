...
int main(int argc, char **argv) {
  ...
  const int ef_fpga_4_update_0_write_DATA_SIZE = num_epochs*2108780;
  const int ef_fpga_4_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t ef_fpga_4_update_0_write_size_bytes = ef_fpga_4_update_0_write_BYTES_PER_PIXEL * ef_fpga_4_update_0_write_DATA_SIZE;

  total_size_bytes += ef_fpga_4_update_0_write_size_bytes;
  total_size_bytes_written += ef_fpga_4_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > ef_fpga_4_update_0_write(ef_fpga_4_update_0_write_size_bytes);
  ...
  for (int i = 0; i < ef_fpga_4_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (ef_fpga_4_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "ef_fpga_4_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer ef_fpga_4_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, ef_fpga_4_update_0_write_size_bytes, ef_fpga_4_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, ef_fpga_4_update_0_write_ocl_buf));
...
nsduration = end - start;
  ...
  std::ofstream regression_result("ef_fpga_4_update_0_write_accel_result.csv");
  for (int i = 0; i < ef_fpga_4_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (ef_fpga_4_update_0_write.data()))[i] << std::endl;
  }
