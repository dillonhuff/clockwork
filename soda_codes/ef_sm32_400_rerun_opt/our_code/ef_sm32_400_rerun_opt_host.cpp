...
int main(int argc, char **argv) {
  ...
  const int ef_sm32_400_rerun_update_0_write_pipe0_DATA_SIZE = num_epochs*2088960;
  const int ef_sm32_400_rerun_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t ef_sm32_400_rerun_update_0_write_pipe0_size_bytes = ef_sm32_400_rerun_update_0_write_pipe0_BYTES_PER_PIXEL * ef_sm32_400_rerun_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += ef_sm32_400_rerun_update_0_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > ef_sm32_400_rerun_update_0_write_pipe0(ef_sm32_400_rerun_update_0_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "ef_sm32_400_rerun_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer ef_sm32_400_rerun_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, ef_sm32_400_rerun_update_0_write_pipe0_size_bytes, ef_sm32_400_rerun_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, ef_sm32_400_rerun_update_0_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("ef_sm32_400_rerun_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < ef_sm32_400_rerun_update_0_write_pipe0_DATA_SIZE; i++) {
