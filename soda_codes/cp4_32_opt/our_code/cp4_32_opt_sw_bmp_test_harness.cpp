...
#include <fstream>
#include <vector>
#include "cp4_32_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<512> > raw_1_merged717_read_channel;
  HWStream<hw_uint<512> > cp4_32_1_merged747_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 2016 / 32; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        raw_1_merged717_read_channel.write(packed);
    }
  }
  cp4_32_opt(raw_1_merged717_read_channel, cp4_32_1_merged747_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = cp4_32_1_merged747_write_channel.read();
  ...
  output.save_image("./images/cp4_32_opt_bmp_out.bmp");
}
