#include <fstream>
#include "heat3d_1_8_opt.h"
#include "clockwork_standard_compute_units.h"

int main() {
  ofstream in_pix("input_pixels_regression_result_heat3d_1_8_opt.txt");
  ofstream fout("regression_result_heat3d_1_8_opt.txt");
  HWStream<hw_uint<256> > in_cc_update_0_read;
  HWStream<hw_uint<256> > heat3d_1_8_update_0_write;


  // Loading input data
  // cmap    : { in_cc_update_0[root = 0, in_cc_0, in_cc_1, in_cc_2] -> in[0, 0] : -1 <= in_cc_0 <= 64 and -1 <= in_cc_1 <= 512 and -1 <= in_cc_2 <= 512 }
  // read map: { in[0, 0] -> in_cc_update_0[root = 0, in_cc_0, in_cc_1, in_cc_2] : -1 <= in_cc_0 <= 64 and -1 <= in_cc_1 <= 512 and -1 <= in_cc_2 <= 512 }
  // rng     : { in_cc_update_0[root = 0, in_cc_0, in_cc_1, in_cc_2] : -1 <= in_cc_0 <= 64 and -1 <= in_cc_1 <= 512 and -1 <= in_cc_2 <= 512 }
  for (int i = 0; i < 17436936; i++) {
    hw_uint<256> in_val;
  ...
  heat3d_1_8_opt(in_cc_update_0_read, heat3d_1_8_update_0_write);

  for (int i = 0; i < 16777216; i++) {
    hw_uint<256> actual = heat3d_1_8_update_0_write.read();
