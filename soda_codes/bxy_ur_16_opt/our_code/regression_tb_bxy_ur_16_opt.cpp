...
int main() {
  ofstream in_pix("input_pixels_regression_result_bxy_ur_16_opt.txt");
  ofstream fout("regression_result_bxy_ur_16_opt.txt");
  HWStream<hw_uint<256> > input_update_0_read;
  HWStream<hw_uint<256> > bxy_ur_16_update_0_write;
  ...
  bxy_ur_16_opt(input_update_0_read, bxy_ur_16_update_0_write);

  for (int i = 0; i < 129600; i++) {
    hw_uint<256> actual = bxy_ur_16_update_0_write.read();
