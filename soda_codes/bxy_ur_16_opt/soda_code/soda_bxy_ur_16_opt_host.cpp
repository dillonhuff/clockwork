...
int main(int argc, char **argv) {
  ...
  const int bxy_ur_16_update_0_write_DATA_SIZE = 2094752;
  ...
  total_size_bytes += bxy_ur_16_update_0_write_size_bytes;
  ...
  std::ifstream input_input_update_0_read("input_arg_input_pixel.csv");
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "bxy_ur_16_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, bxy_ur_16_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = 2094752 / 16;
...
std::cout << "total data (bytes) = " << total_size_bytes << std::endl;
