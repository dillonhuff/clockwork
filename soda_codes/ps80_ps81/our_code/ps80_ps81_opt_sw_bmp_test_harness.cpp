...
#include <fstream>
#include <vector>
#include "ps80_ps81_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<128> > in_off_chip0_update_0_read_channel;
  HWStream<hw_uint<128> > in_off_chip1_update_0_read_channel;
  HWStream<hw_uint<128> > ps80_update_0_write_channel;
  HWStream<hw_uint<128> > ps81_update_0_write_channel;
  // In lanes = 8
  for (int r = 0; r < 1087; r++) {
    for (int cl = 0; cl < 984 / 8; cl++) {
      hw_uint<128> packed;
      {
      int c = 8*cl + 0;
  ...
  ps80_ps81_opt(in_off_chip0_update_0_read_channel, in_off_chip1_update_0_read_channel, ps80_update_0_write_channel, ps81_update_0_write_channel);
  bitmap_image output(960, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 960 / 8; cl++) {
      hw_uint<128> packed;
      auto packed_val = ps80_update_0_write_channel.read();
  ...
  output.save_image("./images/ps80_ps81_opt_bmp_out.bmp");
}
