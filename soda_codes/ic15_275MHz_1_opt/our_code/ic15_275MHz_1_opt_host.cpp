...
int main(int argc, char **argv) {
  ...
  const int ic15_275MHz_1_update_0_write_pipe0_DATA_SIZE = num_epochs*2073600;
  const int ic15_275MHz_1_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t ic15_275MHz_1_update_0_write_pipe0_size_bytes = ic15_275MHz_1_update_0_write_pipe0_BYTES_PER_PIXEL * ic15_275MHz_1_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += ic15_275MHz_1_update_0_write_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > ic15_275MHz_1_update_0_write_pipe0(ic15_275MHz_1_update_0_write_pipe0_size_bytes);
  ...
  for (int i = 0; i < ic15_275MHz_1_update_0_write_pipe0_DATA_SIZE; i++) {
    ((uint16_t*) (ic15_275MHz_1_update_0_write_pipe0.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "ic15_275MHz_1_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer ic15_275MHz_1_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, ic15_275MHz_1_update_0_write_pipe0_size_bytes, ic15_275MHz_1_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, ic15_275MHz_1_update_0_write_pipe0_ocl_buf));
  ...
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({ic15_275MHz_1_update_0_write_pipe0_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
{
    std::ofstream regression_result("ic15_275MHz_1_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < ic15_275MHz_1_update_0_write_pipe0_DATA_SIZE; i++) {
      regression_result << ((uint16_t*) (ic15_275MHz_1_update_0_write_pipe0.data()))[i] << std::endl;
    }
}
