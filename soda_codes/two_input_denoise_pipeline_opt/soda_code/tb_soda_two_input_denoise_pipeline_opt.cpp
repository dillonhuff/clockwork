...
int main() {
  ...
  ap_uint<BURST_WIDTH>* two_input_denoise_pipeline = (ap_uint<BURST_WIDTH>*) malloc(sizeof(ap_uint<BURST_WIDTH>)*num_transfers);
  ...
  write_results_decimal<bits_per_pixel>("soda_two_input_denoise_pipeline_opt_regression_result.csv", two_input_denoise_pipeline, nrows, ncols, transfer_cols);
  free(f_off_chip);
  free(u_off_chip);
  free(two_input_denoise_pipeline);
}
