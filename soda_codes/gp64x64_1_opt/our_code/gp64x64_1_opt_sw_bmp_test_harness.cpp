...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > gp64x64_1_update_0_write_channel;
  // In lanes = 1
  for (int r = 0; r < 1039; r++) {
    for (int cl = 0; cl < 1039 / 1; cl++) {
      hw_uint<16> packed;
      {
      int c = 1*cl + 0;
  ...
  gp64x64_1_opt(in_update_0_read_channel, gp64x64_1_update_0_write_channel);
  bitmap_image output(64, 64);
  for (int r = 0; r < 64; r++) {
    for (int cl = 0; cl < 64 / 1; cl++) {
      auto packed_val = gp64x64_1_update_0_write_channel.read();
  ...
  output.save_image("./images/gp64x64_1_opt_bmp_out.bmp");
}
