...
int main(int argc, char **argv) {
  ...
  const int sbl4_32_1_merged839_write_pipe0_DATA_SIZE = num_epochs*2112064;
  const int sbl4_32_1_merged839_write_BYTES_PER_PIXEL = 16 / 8;
  size_t sbl4_32_1_merged839_write_size_bytes = sbl4_32_1_merged839_write_BYTES_PER_PIXEL * sbl4_32_1_merged839_write_pipe0_DATA_SIZE;

  total_size_bytes += sbl4_32_1_merged839_write_size_bytes;
  total_size_bytes_written += sbl4_32_1_merged839_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > sbl4_32_1_merged839_write_pipe0(sbl4_32_1_merged839_write_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "sbl4_32_opt_dis_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer sbl4_32_1_merged839_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, sbl4_32_1_merged839_write_size_bytes, sbl4_32_1_merged839_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, sbl4_32_1_merged839_write_pipe0_ocl_buf));
  ...
  std::ofstream regression_result("sbl4_32_1_merged839_write_accel_result.csv");
  for (int i = 0; i < sbl4_32_1_merged839_write_pipe0_DATA_SIZE; i++) {
#ifdef __FLOAT_OUTPUT__
    regression_result << bitcast<float, uint16_t>(((uint16_t*) (sbl4_32_1_merged839_write_pipe0.data()))[i]) << std::endl;
#else // __FLOAT_OUTPUT__
