...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > cp_16_update_0_write_channel;
  // In lanes = 1
  ...
  cp_16_opt(raw_update_0_read_channel, cp_16_update_0_write_channel);
  bitmap_image output(1080, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1080 / 1; cl++) {
      auto packed_val = cp_16_update_0_write_channel.read();
  ...
  output.save_image("./images/cp_16_opt_bmp_out.bmp");
}
