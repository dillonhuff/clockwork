...
int main(int argc, char **argv) {
  ...
  const int hrs_1_update_0_write_DATA_SIZE = 2085616;
  const int hrs_1_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t hrs_1_update_0_write_size_bytes = hrs_1_update_0_write_BYTES_PER_PIXEL * hrs_1_update_0_write_DATA_SIZE;

  total_size_bytes += hrs_1_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > hrs_1_update_0_write(hrs_1_update_0_write_size_bytes);
  ...
  for (int i = 0; i < hrs_1_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (hrs_1_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "hrs_1_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer hrs_1_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, hrs_1_update_0_write_size_bytes, hrs_1_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, hrs_1_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = 2085616 / 1;
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({hrs_1_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));

  q.finish();

  std::ofstream regression_result("hrs_1_update_0_write_accel_result.csv");
  for (int i = 0; i < hrs_1_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (hrs_1_update_0_write.data()))[i] << std::endl;
  }
