...
int main() {
  ...
  llf_gs_icd_kernel(gray_out, gray_in_oc, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_llf_gs_icd_regression_result.csv", gray_out, nrows, ncols, transfer_cols);
  free(gray_in_oc);
  free(gray_out);
}
