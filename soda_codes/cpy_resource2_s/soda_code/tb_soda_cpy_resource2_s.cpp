// AUTO GEN SODA TB
#include "cpy_resource2_s_kernel.h"
#include <iostream>
#include <fstream>
...
int main() {
  ...
  cpy_resource2_s_kernel(out, in, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_cpy_resource2_s_regression_result.csv", out, nrows, ncols, transfer_cols);
  free(in);
  free(out);
}
