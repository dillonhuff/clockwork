...
#include <fstream>
#include <vector>
#include "cp9_16_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<256> > raw_1_merged365_read_channel;
  HWStream<hw_uint<256> > cp9_16_1_merged395_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 1968 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        raw_1_merged365_read_channel.write(packed);
    }
  }
  cp9_16_opt(raw_1_merged365_read_channel, cp9_16_1_merged395_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 16; cl++) {
      auto packed_val = cp9_16_1_merged395_write_channel.read();
  ...
  output.save_image("./images/cp9_16_opt_bmp_out.bmp");
}
