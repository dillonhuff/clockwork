...
#include <fstream>
#include <vector>
#include "blur5_2_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<32> > input_1_merged17_read_channel;
  HWStream<hw_uint<32> > blur5_2_1_merged23_write_channel;
  // In lanes = 2
  for (int r = 0; r < 1082; r++) {
    for (int cl = 0; cl < 1922 / 2; cl++) {
      hw_uint<32> packed;
      {
      int c = 2*cl + 0;
      ...
      if (r < input.height() && c < input.width()) {
        ...
        input_1_merged17_read_channel.write(packed);
    }
  }
  blur5_2_opt(input_1_merged17_read_channel, blur5_2_1_merged23_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 2; cl++) {
      auto packed_val = blur5_2_1_merged23_write_channel.read();
  ...
  output.save_image("./images/blur5_2_opt_bmp_out.bmp");
}
