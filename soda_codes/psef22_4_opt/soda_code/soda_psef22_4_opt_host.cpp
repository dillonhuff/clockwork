...
int main(int argc, char **argv) {
  ...
  const int psef22_4_update_0_write_DATA_SIZE = num_epochs*1601484;
  const int psef22_4_update_0_write_BYTES_PER_PIXEL = 16 / 8;
  size_t psef22_4_update_0_write_size_bytes = psef22_4_update_0_write_BYTES_PER_PIXEL * psef22_4_update_0_write_DATA_SIZE;

  total_size_bytes += psef22_4_update_0_write_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > psef22_4_update_0_write(psef22_4_update_0_write_size_bytes);
  ...
  for (int i = 0; i < psef22_4_update_0_write_DATA_SIZE; i++) {
    ((uint16_t*) (psef22_4_update_0_write.data()))[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "psef22_4_opt_kernel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer psef22_4_update_0_write_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, psef22_4_update_0_write_size_bytes, psef22_4_update_0_write.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, psef22_4_update_0_write_ocl_buf));
  ...
  uint64_t transfer_size = num_epochs*(1601484 / 4);
...
nsduration = end - start;
  OCL_CHECK(err, err = q.enqueueMigrateMemObjects({psef22_4_update_0_write_ocl_buf}, CL_MIGRATE_MEM_OBJECT_HOST));
...
printf("Execution time = %f (sec) \n", dsduration);
  std::ofstream regression_result("psef22_4_update_0_write_accel_result.csv");
  for (int i = 0; i < psef22_4_update_0_write_DATA_SIZE; i++) {
    regression_result << ((uint16_t*) (psef22_4_update_0_write.data()))[i] << std::endl;
  }
