...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > sbl_ln_32_update_0_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1082; r++) {
    for (int cl = 0; cl < 1984 / 32; cl++) {
      hw_uint<512> packed;
      {
      int c = 32*cl + 0;
  ...
  sbl_ln_32_opt(img_update_0_read_channel, sbl_ln_32_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = sbl_ln_32_update_0_write_channel.read();
  ...
  output.save_image("./images/sbl_ln_32_opt_bmp_out.bmp");
}
