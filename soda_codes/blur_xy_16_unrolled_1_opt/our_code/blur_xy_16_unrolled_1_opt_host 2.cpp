...
int main(int argc, char **argv) {
  ...
  std::vector<int, aligned_allocator<int>> blur_xy_16_unrolled_1_update_0_write(DATA_SIZE);
  std::vector<int, aligned_allocator<int>> input_update_0_read(DATA_SIZE);
  for (int i = 0; i < DATA_SIZE; i++) {
    blur_xy_16_unrolled_1_update_0_write[i] = 0;
  ...
  std::ofstream regression_result("blur_xy_16_unrolled_1_update_0_write_accel_result.csv");
  for (int i = 0; i < OUT_DATA_SIZE; i++) {
    regression_result << blur_xy_16_unrolled_1_update_0_write[i] << std::endl;
  }
