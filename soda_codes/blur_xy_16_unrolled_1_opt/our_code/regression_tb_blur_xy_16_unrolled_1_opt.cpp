...
int main() {
  ofstream in_pix("input_pixels_regression_result_blur_xy_16_unrolled_1_opt.txt");
  ofstream fout("regression_result_blur_xy_16_unrolled_1_opt.txt");
  HWStream<hw_uint<16> > input_update_0_read;
  HWStream<hw_uint<16> > blur_xy_16_unrolled_1_update_0_write;
  ...
  blur_xy_16_unrolled_1_opt(input_update_0_read, blur_xy_16_unrolled_1_update_0_write);

  for (int i = 0; i < 2073600; i++) {
    hw_uint<16> actual = blur_xy_16_unrolled_1_update_0_write.read();
