...
int main(int argc, char **argv) {
  ...
  const int op_hcompute_hw_output_stencil_write_pipe0_DATA_SIZE = num_epochs*3364;
  ...
  const int op_hcompute_padded16_global_wrapper_stencil_read_pipe0_DATA_SIZE = num_epochs*4096;
  const int op_hcompute_padded16_global_wrapper_stencil_read_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t op_hcompute_padded16_global_wrapper_stencil_read_pipe0_size_bytes = op_hcompute_padded16_global_wrapper_stencil_read_pipe0_BYTES_PER_PIXEL * op_hcompute_padded16_global_wrapper_stencil_read_pipe0_DATA_SIZE;

  total_size_bytes += op_hcompute_padded16_global_wrapper_stencil_read_pipe0_size_bytes;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > op_hcompute_padded16_global_wrapper_stencil_read_pipe0(op_hcompute_padded16_global_wrapper_stencil_read_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "harris_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer op_hcompute_padded16_global_wrapper_stencil_read_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, op_hcompute_padded16_global_wrapper_stencil_read_pipe0_size_bytes, op_hcompute_padded16_global_wrapper_stencil_read_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(0, op_hcompute_padded16_global_wrapper_stencil_read_pipe0_ocl_buf));
