...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<32> > harris_mini_update_0_write_channel;
  // In lanes = 1
  for (int r = 0; r < 36; r++) {
  ...
  harris_mini_naive(img_update_0_read_channel, harris_mini_update_0_write_channel);
  bitmap_image output(32, 32);
  for (int r = 0; r < 32; r++) {
    for (int cl = 0; cl < 32 / 1; cl++) {
      auto packed_val = harris_mini_update_0_write_channel.read();
  ...
  output.save_image("./images/harris_mini_naive_bmp_out.bmp");
}
