...
int main(int argc, char **argv) {
  ...
  const int dn_reconv_update_0_write_pipe0_DATA_SIZE = num_epochs*512;
  const int dn_reconv_update_0_write_pipe0_BYTES_PER_PIXEL = 16 / 8;
  size_t dn_reconv_update_0_write_pipe0_size_bytes = dn_reconv_update_0_write_pipe0_BYTES_PER_PIXEL * dn_reconv_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += dn_reconv_update_0_write_pipe0_size_bytes;
  const int f_update_0_read_pipe0_DATA_SIZE = num_epochs*512;
  const int f_update_0_read_pipe0_BYTES_PER_PIXEL = 16 / 8;
  ...
  const int u_update_0_read_pipe0_DATA_SIZE = num_epochs*720;
  const int u_update_0_read_pipe0_BYTES_PER_PIXEL = 16 / 8;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > dn_reconv_update_0_write_pipe0(dn_reconv_update_0_write_pipe0_size_bytes);
  ...
  OCL_CHECK(err, cl::Buffer dn_reconv_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, dn_reconv_update_0_write_pipe0_size_bytes, dn_reconv_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, dn_reconv_update_0_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("dn_reconv_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < dn_reconv_update_0_write_pipe0_DATA_SIZE; i++) {
      regression_result << ((uint16_t*) (dn_reconv_update_0_write_pipe0.data()))[i] << std::endl;
    }
}
