...
#include <fstream>
#include <vector>
#include "sbl4_16_opt_dis.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<256> > img_1_merged446_read_channel;
  HWStream<hw_uint<256> > sbl4_16_1_merged455_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1082; r++) {
    for (int cl = 0; cl < 1936 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        img_1_merged446_read_channel.write(packed);
    }
  }
  sbl4_16_opt_dis(img_1_merged446_read_channel, sbl4_16_1_merged455_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 16; cl++) {
      auto packed_val = sbl4_16_1_merged455_write_channel.read();
  ...
  output.save_image("./images/sbl4_16_opt_dis_bmp_out.bmp");
}
