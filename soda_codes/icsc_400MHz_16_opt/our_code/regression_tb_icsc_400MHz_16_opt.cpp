#include <fstream>
#include "icsc_400MHz_16_opt.h"

int main() {
  ofstream in_pix("input_pixels_regression_result_icsc_400MHz_16_opt.txt");
  ofstream fout("regression_result_icsc_400MHz_16_opt.txt");
  HWStream<hw_uint<256> > in_update_0_read;
  HWStream<hw_uint<256> > icsc_400MHz_16_update_0_write;


  // Loading input data
  // cmap    : { in_update_0[root = 0, in_0, in_1] -> in_off_chip[0, 0] : -10 <= in_0 <= 129 and 0 <= in_1 <= 1089 }
  // read map: { in_off_chip[0, 0] -> in_update_0[root = 0, in_0, in_1] : -10 <= in_0 <= 129 and 0 <= in_1 <= 1089 }
  // rng     : { in_update_0[root = 0, in_0, in_1] : -10 <= in_0 <= 129 and 0 <= in_1 <= 1089 }
  for (int i = 0; i < 152600; i++) {
    hw_uint<256> in_val;
  ...
  icsc_400MHz_16_opt(in_update_0_read, icsc_400MHz_16_update_0_write);

  for (int i = 0; i < 129600; i++) {
    hw_uint<256> actual = icsc_400MHz_16_update_0_write.read();
