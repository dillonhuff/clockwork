#include <fstream>
#include "lcp_mini_opt.h"

int main() {
  ofstream in_pix("input_pixels_regression_result_lcp_mini_opt.txt");
  ofstream fout("regression_result_lcp_mini_opt.txt");
  HWStream<hw_uint<16> > raw_update_0_read;
  HWStream<hw_uint<16> > lcp_mini_update_0_write;
  ...
  lcp_mini_opt(raw_update_0_read, lcp_mini_update_0_write);

  for (int i = 0; i < 19200; i++) {
    hw_uint<16> actual = lcp_mini_update_0_write.read();
