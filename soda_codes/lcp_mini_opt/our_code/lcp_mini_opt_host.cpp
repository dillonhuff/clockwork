...
int main(int argc, char **argv) {
  ...
  const int lcp_mini_update_0_write_DATA_SIZE = num_epochs*19200;
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "lcp_mini_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, err = krnl_vector_add.setArg(1, lcp_mini_update_0_write_ocl_buf));
