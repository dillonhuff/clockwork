...
// Driver function
void harris_mini_opt(HWStream<hw_uint<32> >& /* get_args num ports = 1 */img_oc, HWStream<hw_uint<32> >& /* get_args num ports = 1 */harris_mini, int num_epochs) {
...
#pragma HLS inline recursive
  ...
  for (int epoch = 0; epoch < num_epochs; epoch++) {
	  // Schedules...
	    ...
	    // harris_mini_update_0 -> [1*d1*1*1 + 1*2,1*d0*1*1 + 1*2,1*15]
...
void harris_mini_opt(HWStream<hw_uint<32> >& /* get_args num ports = 1 */img_oc, HWStream<hw_uint<32> >& /* get_args num ports = 1 */harris_mini) {
  ...
  harris_mini_opt(img_oc, harris_mini, 1);
}
#ifdef __VIVADO_SYNTH__
...
static void write_harris_mini_update_0_write(hw_uint<32>* output, HWStream<hw_uint<32> >& v, const int size) {
  hw_uint<32> burst_reg;
  int num_transfers = harris_mini_update_0_write_num_transfers*size;
  for (int i = 0; i < num_transfers; i++) {
    #pragma HLS pipeline II=1
    burst_reg = v.read();
    output[i] = burst_reg;
