...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<512> > icsc_5s_32_update_0_write_channel;
  // In lanes = 32
  for (int r = 0; r < 1085; r++) {
    for (int cl = 0; cl < 2240 / 32; cl++) {
      hw_uint<512> packed;
      {
      int c = 32*cl + 0;
  ...
  icsc_5s_32_opt(in_update_0_read_channel, icsc_5s_32_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 32; cl++) {
      auto packed_val = icsc_5s_32_update_0_write_channel.read();
  ...
  output.save_image("./images/icsc_5s_32_opt_bmp_out.bmp");
}
