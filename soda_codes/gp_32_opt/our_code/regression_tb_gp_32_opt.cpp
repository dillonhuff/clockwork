...
int main() {
  ofstream in_pix("input_pixels_regression_result_gp_32_opt.txt");
  ofstream fout("regression_result_gp_32_opt.txt");
  HWStream<hw_uint<512> > in_update_0_read;
  HWStream<hw_uint<32> > gp_32_update_0_write;


  // Loading input data
  // cmap    : { in_update_0[root = 0, in_0, in_1] -> in_off_chip[0, 0] : 0 <= in_0 <= 5 and 0 <= in_1 <= 78 }
  // read map: { in_off_chip[0, 0] -> in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 5 and 0 <= in_1 <= 78 }
  // rng     : { in_update_0[root = 0, in_0, in_1] : 0 <= in_0 <= 5 and 0 <= in_1 <= 78 }
  for (int i = 0; i < 474; i++) {
    hw_uint<512> in_val;
  ...
  gp_32_opt(in_update_0_read, gp_32_update_0_write);

  for (int i = 0; i < 8; i++) {
    hw_uint<32> actual = gp_32_update_0_write.read();
