...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<16> > sbl_ln_1_update_0_write_channel;
  // In lanes = 1
  ...
  sbl_ln_1_opt(img_update_0_read_channel, sbl_ln_1_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 1; cl++) {
      auto packed_val = sbl_ln_1_update_0_write_channel.read();
  ...
  output.save_image("./images/sbl_ln_1_opt_bmp_out.bmp");
}
