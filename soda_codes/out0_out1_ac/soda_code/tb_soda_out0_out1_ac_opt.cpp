// AUTO GEN SODA TB
#include "out0_out1_ac_opt_kernel.h"
#include <iostream>
#include <fstream>
...
int main() {
  ...
  ap_uint<BURST_WIDTH>* out1_ac = (ap_uint<BURST_WIDTH>*) malloc(sizeof(ap_uint<BURST_WIDTH>)*num_transfers);
  ...
  out0_out1_ac_opt_kernel(out0, out1_ac, in0_oc, in1_oc, num_transfers);
  write_results_decimal<bits_per_pixel>("soda_out0_out1_ac_opt_regression_result.csv", out0, nrows, ncols, transfer_cols);
  write_results_decimal<bits_per_pixel>("soda_out0_out1_ac_opt_regression_result.csv", out1_ac, nrows, ncols, transfer_cols);
  free(in0_oc);
  free(in1_oc);
  free(out0);
  free(out1_ac);
