...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<256> > raw_1_merged371_read_channel;
  HWStream<hw_uint<256> > cp4_16_1_merged383_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 1968 / 16; cl++) {
      ...
      if (r < input.height() && c < input.width()) {
        ...
        raw_1_merged371_read_channel.write(packed);
    }
  }
  cp4_16_opt(raw_1_merged371_read_channel, cp4_16_1_merged383_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 16; cl++) {
      auto packed_val = cp4_16_1_merged383_write_channel.read();
  ...
  output.save_image("./images/cp4_16_opt_bmp_out.bmp");
}
