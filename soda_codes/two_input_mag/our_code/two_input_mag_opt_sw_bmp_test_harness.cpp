...
int main(int argc, char **argv) {
  ...
  HWStream<hw_uint<32> > two_input_mag_update_0_write_channel;
  // In lanes = 1
  for (int r = 0; r < 32; r++) {
  ...
  for (int r = 0; r < 30; r++) {
    for (int cl = 0; cl < 30 / 1; cl++) {
      auto packed_val = two_input_mag_update_0_write_channel.read();
  ...
  output.save_image("./images/two_input_mag_opt_bmp_out.bmp");
}
