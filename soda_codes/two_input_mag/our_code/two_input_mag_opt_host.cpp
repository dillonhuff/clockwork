...
int main(int argc, char **argv) {
  ...
  const int f_update_0_read_pipe0_DATA_SIZE = num_epochs*1024;
  ...
  const int two_input_mag_update_0_write_pipe0_DATA_SIZE = num_epochs*900;
  const int two_input_mag_update_0_write_pipe0_BYTES_PER_PIXEL = 32 / 8;
  size_t two_input_mag_update_0_write_pipe0_size_bytes = two_input_mag_update_0_write_pipe0_BYTES_PER_PIXEL * two_input_mag_update_0_write_pipe0_DATA_SIZE;

  total_size_bytes += two_input_mag_update_0_write_pipe0_size_bytes;
  const int u_update_0_read_pipe0_DATA_SIZE = num_epochs*1024;
  ...
  std::vector<uint8_t, aligned_allocator<uint8_t> > two_input_mag_update_0_write_pipe0(two_input_mag_update_0_write_pipe0_size_bytes);
  ...
  for (unsigned int i = 0; i < devices.size(); i++) {
    ...
    if (err != CL_SUCCESS) {
      ...
      OCL_CHECK(err, krnl_vector_add = cl::Kernel(program, "two_input_mag_opt_accel", &err));
      valid_device++;
      break;
    }
  }
  ...
  OCL_CHECK(err, cl::Buffer two_input_mag_update_0_write_pipe0_ocl_buf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, two_input_mag_update_0_write_pipe0_size_bytes, two_input_mag_update_0_write_pipe0.data(), &err));
  OCL_CHECK(err, err = krnl_vector_add.setArg(2, two_input_mag_update_0_write_pipe0_ocl_buf));
...
{
    std::ofstream regression_result("two_input_mag_update_0_write_pipe0_accel_result.csv");
    for (int i = 0; i < two_input_mag_update_0_write_pipe0_DATA_SIZE; i++) {
      regression_result << ((uint32_t*) (two_input_mag_update_0_write_pipe0.data()))[i] << std::endl;
    }
}
