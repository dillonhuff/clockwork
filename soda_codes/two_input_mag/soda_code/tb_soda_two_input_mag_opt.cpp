...
int main() {
  ...
  two_input_mag_opt_kernel(two_input_mag, f_off_chip, u_off_chip, transfer_cols);
  write_results<bits_per_pixel>("soda_two_input_mag_opt_regression_result.csv", two_input_mag, nrows, ncols, transfer_cols);
  free(f_off_chip);
  free(u_off_chip);
  free(two_input_mag);
}
