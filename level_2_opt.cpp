...
struct in_in_comp_write0_cache {
	...
	inline hw_uint<32>  peek(const int offset) {
		...
		cout << "Error: Unsupported offset in in: " << offset << endl;
...
#ifndef __VIVADO_SYNTH__
		cout << "Error: Unsupported offset in level_0: " << offset << endl;
#endif // __VIVADO_SYNTH__
		assert(false);
		return 0;
...
#ifndef __VIVADO_SYNTH__
		cout << "Error: Unsupported offset in level_1: " << offset << endl;
#endif // __VIVADO_SYNTH__
		assert(false);
		return 0;
...
#pragma HLS pipeline II=1
    ...
    if ((0 <= c1 && c1 <= 262) && (0 <= c0 && c0 <= 262)) {
      in_comp(c1, c0);
    }

    if ((2 <= c1 && c1 <= 262) && (2 <= c0 && c0 <= 262)) {
      level_0_comp(c1, c0);
    }

    if ((6 <= c1 && c1 <= 262) && (6 <= c0 && c0 <= 262)) {
      level_1_comp(c1, c0);
    }

    if ((14 <= c1 && c1 <= 262) && (14 <= c0 && c0 <= 262)) {
      level_2_comp(c1, c0);
    }
