...
#include <fstream>
#include <vector>
#include "infer_bounds_unroll.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<128> > pw_math_in_oc02_merged66_read_channel;
  HWStream<hw_uint<128> > pw_math_c68_merged72_write_channel;
  // In lanes = 4
  for (int r = 0; r < 18; r++) {
    for (int cl = 0; cl < 20 / 4; cl++) {
      hw_uint<128> packed;
      {
      int c = 4*cl + 0;
      ...
      if (r < input.height() && c < input.width()) {
        ...
        pw_math_in_oc02_merged66_read_channel.write(packed);
    }
  }
  infer_bounds_unroll(pw_math_in_oc02_merged66_read_channel, pw_math_c68_merged72_write_channel);
  bitmap_image output(16, 16);
  for (int r = 0; r < 16; r++) {
    for (int cl = 0; cl < 16 / 4; cl++) {
      auto packed_val = pw_math_c68_merged72_write_channel.read();
  ...
  output.save_image("./images/infer_bounds_unroll_bmp_out.bmp");
}
