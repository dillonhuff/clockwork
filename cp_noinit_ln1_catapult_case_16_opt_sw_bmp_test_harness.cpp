...
#include <fstream>
#include <vector>
#include "cp_noinit_ln1_catapult_case_16_opt.h"
#include "bitmap_image.hpp"
#include <cstdlib>

int main(int argc, char **argv) {
  bitmap_image input("./images/taxi_slice_256.bmp");
  HWStream<hw_uint<256> > raw_update_0_read_channel;
  HWStream<hw_uint<256> > cp_noinit_ln1_catapult_case_16_update_0_write_channel;
  // In lanes = 16
  for (int r = 0; r < 1088; r++) {
    for (int cl = 0; cl < 2000 / 16; cl++) {
      hw_uint<256> packed;
      {
      int c = 16*cl + 0;
      ...
      if (r < input.height() && c < input.width()) {
        ...
        raw_update_0_read_channel.write(packed);
    }
  }
  cp_noinit_ln1_catapult_case_16_opt(raw_update_0_read_channel, cp_noinit_ln1_catapult_case_16_update_0_write_channel);
  bitmap_image output(1920, 1080);
  for (int r = 0; r < 1080; r++) {
    for (int cl = 0; cl < 1920 / 16; cl++) {
      auto packed_val = cp_noinit_ln1_catapult_case_16_update_0_write_channel.read();
  ...
  output.save_image("./images/cp_noinit_ln1_catapult_case_16_opt_bmp_out.bmp");
}
