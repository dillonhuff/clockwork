...
void generate_xilinx_accel_raw_wrapper(CodegenOptions& options, std::ostream& out, map<string, UBuffer>& buffers, prog& prg) {
  cout << "Generating accel wrapper" << endl;
  string driver_func = prg.name + "_raw";
...
void generate_app_code(CodegenOptions& options,
  ...
  generate_xilinx_accel_raw_wrapper(options, conv_out, buffers, prg);
  close_synth_scope(conv_out);
