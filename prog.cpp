...
void generate_app_code_op_logic(
  ...
  for (auto& b : buffers) {
    ...
    ofstream a(prg.name+ "_check_codegen_c.txt");
  a << "1" << endl;
  string code_string =
    options.code_string;
...
// a.close();
  ...
  for (auto s : get_maps(schedmap)) {
    ...
    a << codegen_c(range(s))  << endl << endl << endl;
  }
 
  conv_out << endl;
  conv_out << code_string << endl;
  
  a << code_string << endl << endl;  
...
void generate_app_code_op_logic_catapult(
  ...
  for (auto& b : buffers) {
   ...
   ofstream a(prg.name+ "_check_codegen_catapult.txt");
  a << "1" << endl;
...
void generate_regression_testbench_catapult(prog& prg, map<string, UBuffer>& buffers) {
  ...
  rgtb << "#include \"" << prg.name << ".h\"" << endl;
  rgtb << "#include \"clockwork_standard_compute_units_catapult.h\"" << endl << endl;
