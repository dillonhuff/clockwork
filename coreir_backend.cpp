...
void run_lake_verilog_codegen(CodegenOptions& options, string v_name, string ub_ins_name) {
  ...
  ASSERT(getenv("LAKE_SCRIPT_DIR"), "Define env var $LAKE_SCRIPT_DIR which is the /PathTo/lake/tests");
  int res_lake = cmd("python $LAKE_SCRIPT_DIR/wrapper_lake.py -c " + options.dir + "lake_collateral/" + ub_ins_name + " -s True -n " + v_name);
  assert(res_lake == 0);
