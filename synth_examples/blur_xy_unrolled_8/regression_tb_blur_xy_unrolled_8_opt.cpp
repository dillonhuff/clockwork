#include <fstream>
#include "blur_xy_unrolled_8_opt.h"

int main() {
  ofstream in_pix("input_pixels_regression_result_blur_xy_unrolled_8_opt.txt");
  ofstream fout("regression_result_blur_xy_unrolled_8_opt.txt");
  HWStream<hw_uint<256> > input_update_0_read;
  HWStream<hw_uint<256> > blur_xy_unrolled_8_update_0_write;


  // Loading input data
  // cmap    : { input_update_0[root = 0, input_0, input_1] -> input_arg[0, 0] : 0 <= input_0 <= 240 and 0 <= input_1 <= 1081 }
  // read map: { input_arg[0, 0] -> input_update_0[root = 0, input_0, input_1] : 0 <= input_0 <= 240 and 0 <= input_1 <= 1081 }
  // rng     : { input_update_0[root = 0, input_0, input_1] : 0 <= input_0 <= 240 and 0 <= input_1 <= 1081 }
  for (int i = 0; i < 260762; i++) {
    hw_uint<256> in_val;
    in_pix << 8*i + 0 << endl;
    set_at<0*32, 256>(in_val, 8*i + 0);
    in_pix << 8*i + 1 << endl;
    set_at<1*32, 256>(in_val, 8*i + 1);
    in_pix << 8*i + 2 << endl;
    set_at<2*32, 256>(in_val, 8*i + 2);
    in_pix << 8*i + 3 << endl;
    set_at<3*32, 256>(in_val, 8*i + 3);
    in_pix << 8*i + 4 << endl;
    set_at<4*32, 256>(in_val, 8*i + 4);
    in_pix << 8*i + 5 << endl;
    set_at<5*32, 256>(in_val, 8*i + 5);
    in_pix << 8*i + 6 << endl;
    set_at<6*32, 256>(in_val, 8*i + 6);
    in_pix << 8*i + 7 << endl;
    set_at<7*32, 256>(in_val, 8*i + 7);
    input_update_0_read.write(in_val);
  }

  blur_xy_unrolled_8_opt(input_update_0_read, blur_xy_unrolled_8_update_0_write);

  for (int i = 0; i < 259200; i++) {
    hw_uint<256> actual = blur_xy_unrolled_8_update_0_write.read();
    auto actual_lane_0 = actual.extract<0*32, 31>();
    fout << (int) actual_lane_0 << endl;
    auto actual_lane_1 = actual.extract<1*32, 63>();
    fout << (int) actual_lane_1 << endl;
    auto actual_lane_2 = actual.extract<2*32, 95>();
    fout << (int) actual_lane_2 << endl;
    auto actual_lane_3 = actual.extract<3*32, 127>();
    fout << (int) actual_lane_3 << endl;
    auto actual_lane_4 = actual.extract<4*32, 159>();
    fout << (int) actual_lane_4 << endl;
    auto actual_lane_5 = actual.extract<5*32, 191>();
    fout << (int) actual_lane_5 << endl;
    auto actual_lane_6 = actual.extract<6*32, 223>();
    fout << (int) actual_lane_6 << endl;
    auto actual_lane_7 = actual.extract<7*32, 255>();
    fout << (int) actual_lane_7 << endl;
  }

  in_pix.close();
  fout.close();
  return 0;
}
